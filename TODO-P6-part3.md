...
## [DONE] P6-T04：接通 GC roots / stackmaps / runtime 语义，并锁定 dropped continuation、runtime error 与 Managed ABI 边界
...
- 依赖：P6-T03R
- 完成记录：
  - 2026-05-05：refactor frame / continuation object allocation 已从 raw `malloc` 收口到 `scoop_alloc_typed`，并为 refactor frame / continuation struct 生成 GC type descriptor；payload 字段清零不再覆盖 runtime 初始化的 GC header。
  - 2026-05-05：refactor callable body 现在为 frame root、resume continuation 参数和新建 continuation 建立 explicit root slot，并在 refactor direct / resume / dynamic call 与 runtime allocation/write-barrier call 边界通过 root-preserving path 同步 moving-GC roots；frame field / continuation captured-frame / composed-callee stores 使用 GC-aware store 或 write barrier。
  - 2026-05-05：补齐直接阻塞 P6-T04 验证的 handoff 修正：handle arm completion payload source 现在按 arm region 中通向 completion target 的 state 查找真实完成值，避免把无关副作用赋值误判为 payload；builtin `String.concat` 在 effect facts 和 refactor body lowering 中按 pure runtime primitive 处理，避免落入 dynamic fallback 并制造错误 outward case；自包含 handle 的 escaped continuation 也会纳入 compiler-generated runtime-error upper bound，保证 double resume 仍有 ordinary runtime-error `Step_F` case。
  - 2026-05-05：新增 `refactor_llvm_gc_roots`、`refactor_llvm_stackmap`、`refactor_llvm_dropped_continuation`、`refactor_llvm_managed_abi_boundary` 定向测试入口，并新增 `tests/fixtures/build/effect_refactor_no_legacy_handler_stack_calls.scoop` 锁定 typed GC allocation、continuation type descriptor、write barrier 与无 legacy handler-stack/outcome calls。
...
## [DONE] P6-T05：建立 refactor LLVM 定向 build/run-pass/runtime_gc 验证矩阵，并冻结 P6 -> P7 handoff contract
...
- 依赖：P6-T04R
- 完成记录：
  - 2026-05-05：建立 P6 -> P7 handoff 的定向验证矩阵，覆盖 build `NoOutward` / `SingleCase` / `CanonicalFull`、direct perform/handle/resume、dynamic callable publication、Unit 零载荷、no legacy handler-stack/outcome、`--emit-obj` 与 `--emit-asm -O2` artifact helper；`EFFECT_REFACTOR.md` §5.6.6 已固化矩阵与“P7 只做 selector flip + full regression”的边界。
  - 2026-05-05：`scoop build --emit-*`、build fixtures 与 `scoop run` 的 executable build 路径统一经 `emit_refactor_llvm_artifact_for_build` 进入 `effect_refactor_pipeline::emit_production_llvm_artifact_to_file`，避免 fixture/产物种类保留单独 lowering route。
  - 2026-05-05：新增 runtime_gc 跨线程 resume 样本 `effect_cross_thread_resume_payload_refs.scoop`，在 moving-GC stress 下覆盖 escaped continuation、captured String ref、resume payload ref 与跨线程 writeback；refactor lowering 现在通过 `scoop_thread_spawn_join_refactor_resume_u64` + compiler-generated surface-resume thunk 调用 published refactor continuation ABI，不再把该 helper 降到 legacy `scoop_continuation_resume_u64`。
  - 2026-05-05：补齐 P6-T05 样本直接暴露的 refactor value primitive 缺口：tuple literal / tuple member lowering、whole-tuple resume payload packing、core print/toString runtime primitive 与跨线程 resume helper，均保持在 effect-neutral primitive 或 published surface-resume contract 边界内。
...
## [DONE] P6-T05a：闭合 `NoOutward` plain callable 对本地 effect/control body 的 handoff，禁止 P6-T06 用 legacy fallback 或 complete-only `Step_F` 绕过
...
- 完成记录：
  - 2026-05-05：`LateLoweredPlainCallable` 新增可选 plain-local effect/control handoff；`NoOutward` plain callable 的公开 ABI 仍是普通函数 ABI，但当 body 内含本地 `handle` / `perform` / `resume` / runtime-error catch 时，P5 会发布 local state graph、frame schema、boundary map、resume map、continuation object、resume packing 与 source statement classification。
  - 2026-05-05：P5 plain call-site 收集现在只记录 ordinary `Call` facts，不再把 `ResumeSiteEffectFacts` / `PerformSiteEffectFacts` / `HandleSiteEffectFacts` 强塞进 `LateLoweredPlainCallSite`；plain-local handoff 会从 handle/perform continuation schema 发现唯一 owner `StepSchema`，并对缺失、歧义或 return/complete type 漂移 fail fast。
  - 2026-05-05：P6 ABI materialization / surface-resume inventory / frame、boundary、resume payload、completion payload、HandleDispatch 与 local runtime-error layout 均消费 `has_control_body()`，覆盖 plain-local control body；post-opt 保留 plain-local continuation object 与 resume packing。
  - 2026-05-05：P6 plain emitter 在 plain callable 内有 local handoff 时使用 `emit_plain_direct` lower published state graph，并在 `Return` state 回到普通函数返回值；若 NoOutward local path 尝试向外返回 `Step_F` 会 fail fast。root `main` 的 plain source entry 改用内部 refactor symbol，避免与 C `main` wrapper 符号冲突。
  - 2026-05-05：补齐 plain pure value primitive 对 builtin `ToString.toString` interface call 的 effect-neutral lowering，保证 plain `println<Int>` 等 helper 不需要 legacy statement/function fallback。
...
## [DONE] P6-T06R：Review `NoOutward` plain ABI 修复，确认 P7 前不再存在 complete-only `Step_F` 回归
...
- 依赖：P6-T06
- 完成记录：
  - 2026-05-05：完成 `NoOutward` plain ABI review。确认 P4 `CallableAbiKind::Plain` 仍不发布 callable body `StepSchema`，P5 `LateLoweredCallableAbi::Plain` 仍不发布公开 `Step_F` / dynamic invoke / continuation shell，P6 plain callable layout 与 body emitter 继续走普通函数 ABI；effect-typed plain closure adapter 与 true effectful callable 仍使用独立 `EffectStep` surface。
  - 2026-05-05：review 验证发现并修复直接阻断项：pure/plain caller（如 `main -> run(...)`）调用 `EffectStep` callee 并本地消费 compiler-generated runtime-error case 时，P4/P5 在 plain callable 上缺少内部 local-control owner schema，导致 `refactor_effect_lowered_stage` 与 `effect_resume_if_else_branch_single_perform.scoop` 失败。现已在 `BodyEffectFacts` 中发布 `local_control_step_schema`，仅供 plain-local control lowering 使用；它不会改变 callable 的公开 `Plain` ABI，也不会把 `NoOutward` body 退回 complete-only `Step_F`。
  - 2026-05-05：review 搜索命中仅包含设计文档/历史 DONE 任务记录、`effect_refactor_step_enum_no_outward.scoop` 的负向 fixture 说明，以及 effect-step 路径的 fail-fast 诊断（如 direct entry 未返回 `Step_F`）；未发现新的 complete-only `NoOutward` 合同或 plain callable `Step_F` 回归。
  - 验证通过：`cargo test -p scoopc --no-default-features refactor_callable_effect_facts_no_outward`；`cargo test -p scoopc --no-default-features refactor_step_schema_not_published_for_plain_body`；`cargo test -p scoopc --no-default-features refactor_call_site_facts_distinguish_plain_call_and_effect_adapter`；`cargo test -p scoopc --no-default-features refactor_effect_solver`；`cargo test -p scoop --no-default-features dump_effect_facts`；`cargo run -p scoop --no-default-features -- --effect-pipeline refactor dump-effect-facts tests/fixtures/effect_facts/direct_and_fun_value_call.scoop`。
  - 验证通过：`cargo test -p scoopc --no-default-features refactor_effect_lowered_no_outward_plain_callable`；`cargo test -p scoopc --no-default-features refactor_late_lowered_ir`；`cargo test -p scoopc --no-default-features refactor_source_slice_plain_call_keeps_ordinary_call_contract`；`cargo test -p scoopc --no-default-features refactor_effect_lowered_stage`；`cargo test -p scoopc --no-default-features refactor_continuation_object`；`cargo test -p scoopc --no-default-features refactor_resume_interface_completeness`；`cargo test -p scoopc --no-default-features refactor_impl_plan_lowering_keeps_no_outward`；`cargo run -p scoop --no-default-features -- --effect-pipeline refactor dump-effect-lowered tests/fixtures/effect_lowered/direct_and_fun_value_call.scoop`。
  - 验证通过：`cargo test -p scoopc refactor_llvm_no_outward_plain_abi`；`cargo test -p scoopc refactor_llvm_plain_call_lowering`；`cargo test -p scoopc refactor_llvm_dynamic_invoke_lowering`；`cargo test -p scoopc refactor_llvm_effect_typed_adapter_wraps_plain_body`；`cargo test -p scoopc refactor_llvm_plain_local_effect_control`；`cargo test -p scoopc refactor_effect_lowered_plain_local_effect_control`；`cargo run -p scoop -- --effect-pipeline refactor test --fixtures tests/fixtures/build/effect_refactor_step_enum_no_outward.scoop`；`cargo run -p scoop -- --effect-pipeline refactor test --fixtures tests/fixtures/build/effect_refactor_step_enum_single_case.scoop`；`cargo run -p scoop -- --effect-pipeline refactor test --fixtures tests/fixtures/build/effect_refactor_step_enum_canonical_full_O0.scoop`；`cargo run -p scoop -- --effect-pipeline refactor test --fixtures tests/fixtures/run-pass/continuation_resume_surface_named_tuple_and_unit_basic.scoop`；`cargo run -p scoop -- --effect-pipeline refactor test --fixtures tests/fixtures/build/effect_refactor_non_boundary_dynamic_call_emit_llvm.scoop`；`cargo run -p scoop -- --effect-pipeline refactor test --fixtures tests/fixtures/build/effect_refactor_dynamic_entry_publication_emit_llvm.scoop`。
  - 额外阻断回归验证通过：`cargo run -p scoop -- --effect-pipeline refactor test --fixtures tests/fixtures/run-pass/effect_resume_if_else_branch_single_perform.scoop`。
  - 搜索与 lint 通过：`rg "Complete-only|complete-only|NoOutward.*Complete|step_enum_no_outward|direct entry 未返回 Step_F|pure statement call.*Step_F" EFFECT_REFACTOR.md PLAN.md TODO-P*.md crates/scoopc/src/llvm/codegen/effect_refactor tests/fixtures/build`；`cargo clippy -p scoopc -p scoop --no-default-features --all-targets -- -D warnings`；`cargo clippy --all-targets -- -D warnings`。

## [DONE] P6-T05R：Review P6 阶段退出条件，确认 P7 只需切主线并执行 full regression
...
- 依赖：P6-T06R
- 完成记录：
  - 2026-05-05：完成 P6 阶段退出 review。确认 refactor LLVM codegen stage 已作为 build/run/fixture/emit-* 的共同入口，body lowering 仍按 clean backend 边界消费 P5/P6 published contract 与 effect-neutral value primitive；`Step_F` / frame / continuation object / resume surface / wrapper projection / call invoke / HandleDispatch / runtime error / GC roots 均由 ABI query 与 late-lowered handoff 驱动。
  - 2026-05-05：review 中修复 P6 exit 矩阵直接阻断项：`effect_refactor_dynamic_invoke_unit_payload.scoop` 原先只声明 `unitWorker(): Unit / Ping {}`，在 `NoOutward -> Plain` 合同下正确降为普通 ABI，已改为真实执行 `Ping.tick()`，继续覆盖 Unit 零载荷 dynamic/direct invoke 与 resume ABI，而不依赖旧 complete-only shell。
  - 2026-05-05：review 中修复 placeholder runtime 终止项：cross-thread surface resume thunk 的 non-Complete 分支不再调用 `scoop_runtime_error_fatal(NULL)`，改为新增显式 runtime terminal `scoop_refactor_thread_resume_noncomplete_fatal()`，避免用 null payload 伪造 ordinary RuntimeError。
  - 2026-05-05：搜索守卫确认 clean backend 中 `null_payload` / legacy statement fallback / known placeholder 文本无命中；legacy handler-stack/outcome 命中仅来自 legacy backend 自身，`effect_op_call_sites` / `hir::HandleExpr` 命中仅为 typed-HIR contract publication 与 ABI test scaffolding，complete-only `NoOutward` 命中仅为设计文档、历史 DONE 记录、负向 fixture 说明或 effect-step fail-fast 诊断。
  - 验证通过：本文件 `P6-T02qh` ~ `P6-T06R` 的定向 cargo test / fixture / dump / build / run / runtime_gc 矩阵，包括 `refactor_effect_lowered_call_boundary_continuation_composition`、`refactor_llvm_call_boundary_continuation_composition`、`refactor_effect_lowered_surface_resume_wrapper_completion`、`refactor_llvm_surface_resume_wrapper_completion`、`refactor_llvm_clean_backend_boundary`、`refactor_llvm_value_primitive`、`refactor_effect_lowered_source_slice_classification`、`refactor_llvm_source_slice_classification`、`refactor_llvm_pure_statement_lowering`、`refactor_llvm_member_read_write_lowering`、`refactor_llvm_function_abi`、`refactor_llvm_main_wrapper`、`refactor_llvm_call_lowering`、`refactor_llvm_dynamic_invoke_lowering`、`refactor_llvm_boundary_lowering`、`refactor_llvm_runtime_error_case`、`refactor_llvm_handle_dispatch_lowering`、`refactor_llvm_handle_pending_completion`、`refactor_llvm_continuation_protocol`、`refactor_llvm_double_resume_runtime_error`、`refactor_llvm_body_verifier`、`refactor_llvm_runtime_error_lowering`、`refactor_llvm_gc_roots`、`refactor_llvm_stackmap`、`refactor_llvm_dropped_continuation`、`refactor_llvm_managed_abi_boundary`、`refactor_effect_lowered_plain_local_effect_control`、`refactor_llvm_plain_local_effect_control`、`refactor_llvm_no_outward_plain_abi`、`refactor_llvm_plain_call_lowering`、`refactor_llvm_effect_typed_adapter_wraps_plain_body` 与 `cargo test -p scoop build_fixtures_propagate_refactor_session_options_to_build_command`。
  - 验证通过：no-default P4/P5/P6-T06R handoff 命令，包括 `refactor_callable_effect_facts_no_outward`、`refactor_step_schema_not_published_for_plain_body`、`refactor_call_site_facts_distinguish_plain_call_and_effect_adapter`、`refactor_effect_solver`、`dump_effect_facts`、`dump-effect-facts tests/fixtures/effect_facts/direct_and_fun_value_call.scoop`、`refactor_effect_lowered_no_outward_plain_callable`、`refactor_late_lowered_ir`、`refactor_source_slice_plain_call_keeps_ordinary_call_contract`、`refactor_effect_lowered_stage`、`refactor_continuation_object`、`refactor_resume_interface_completeness`、`refactor_impl_plan_lowering_keeps_no_outward` 与 `dump-effect-lowered tests/fixtures/effect_lowered/direct_and_fun_value_call.scoop`。
  - 验证通过：refactor CLI fixtures / artifacts，包括 `dump-effect-lowered` 的 `effect_multi_escape_indirect_direct_while.scoop`、`effect_resume_if_else_branch_single_perform.scoop`、`continuation_resume_surface_named_tuple_and_unit_basic.scoop`；build fixtures `effect_refactor_step_enum_no_outward.scoop`、`effect_refactor_step_enum_single_case.scoop`、`effect_refactor_step_enum_canonical_full_O0.scoop`、`effect_refactor_direct_handle_resume_emit_llvm.scoop`、`effect_refactor_dynamic_entry_publication_emit_llvm.scoop`、`effect_refactor_non_boundary_dynamic_call_emit_llvm.scoop`、`effect_refactor_dynamic_invoke_unit_payload.scoop`、`effect_refactor_no_legacy_handler_stack_calls.scoop`、`effect_refactor_emit_obj_smoke.scoop`、`effect_refactor_emit_asm_O2.scoop`；run-pass fixtures `effect_resume_double_resume_exit.scoop`、`effect_multi_escape_indirect_direct_while.scoop`、`continuation_resume_surface_named_tuple_and_unit_basic.scoop`、`effect_resume_if_else_branch_single_perform.scoop`、`handle_finally_boundary.scoop`、`effect_resume_finally_normal.scoop`、`effect_escape_continuation_resume_later_exit.scoop`；`build --emit-llvm tests/fixtures/run-pass/effect_resume_double_resume_exit.scoop -o /tmp/p6_refactor_resume.ll`；`run tests/fixtures/run-pass/continuation_resume_surface_named_tuple_and_unit_basic.scoop`。
  - 验证通过：moving-GC runtime fixtures `SCOOP_GC_MOVE=1 SCOOP_GC_STRESS=1 SCOOP_GC_VERIFY_ROOTS=1 cargo run -p scoop -- --effect-pipeline refactor test --fixtures tests/fixtures/runtime_gc/effect_outer_mutable_state_body_writeback_basic.scoop` 与 `tests/fixtures/runtime_gc/effect_cross_thread_resume_payload_refs.scoop`；指定 `rg` 搜索守卫；`cargo clippy -p scoopc -p scoop --no-default-features --all-targets -- -D warnings`；`cargo clippy --all-targets -- -D warnings`。
  - 结论：P6 clean refactor LLVM codegen 对接已完成；P7 可以只做 selector 默认值翻转与 full regression，不需要重新讨论 LLVM effect backend 架构、ABI、GC/runtime 或 legacy fallback 边界。
