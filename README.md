...
- `crates/scoopc_lir_facts/`：独立 LIR facts 数据产品；承载 LIR stage summary、callable ABI、dynamic invoke、dispatch owner/slot、continuation/resume publication contracts 与 LIR opt pipeline metadata；只依赖基础 crate，不依赖 `scoopc` facade、effect/MIR/LIR stage 或 backend crate；是 TODO-6/P7 backend cleanup 的 backend-neutral 输入基础
