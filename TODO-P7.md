...
- 参考：
  ...
  - [`TODO-P7.md`](./TODO-P7.md) P7-T02Z
- 背景：
  - 继续收口 P7-T02Z 默认 run-pass 时，`effect_multi_escape_custom_nonresuming_direct_indirect_multi.scoop` 暴露：两个不同 owner continuation object（例如 `run_direct_indirect_direct` 与 `run_indirect_direct`）可共享同一个 owner-trampoline surface resume schema（如 k6/k7）；
  - 当前 refactor LLVM ABI materializer 的 `surface-resume owner dispatch` contract 仍假定一个 schema 只能落到单一 owner continuation object；
  - late-lowered wrapper projection 也把 owner-specific completion payload source 绑定进 schema 级唯一 projection，导致多 owner projection 被视作冲突或在 ABI materialization 中变成 owner 歧义。

- 必须实现的内容：
  1. 在 late-lowered handoff 中发布 multi-owner owner-trampoline surface-resume dispatch contract。
     - 同一 `ContinuationSchemaId` 若来自多个 owner continuation object，必须显式列出每个 owner 的 continuation object、owner version、resume boundary / handle binder route、以及对应 wrapper projection；
     - 禁止在 P6/P7 LLVM backend 现场按“第一个 owner”猜测，或忽略后续 owner。
  2. 将 wrapper projection 从 schema 级单值扩展为 owner-aware 表达。
     - completion payload source 若引用 owner-local local/frame slot，必须保留 owner 维度；
     - owner-step -> wrapper-step 的 case projection 可共享形状，但不能丢失 owner-specific completion payload source。
  3. 更新 ABI materializer / query / body lowering。
     - surface resume entry 必须能根据实际 continuation object 选择正确 owner dispatch；
     - resume boundary 与 handle continuation binder 的 route key 不得只用 site id / arm / case 造成跨 owner 冲突；
     - `Continuation<..., ...>` surface resume 不得因为多个 owner 共享 schema 而 fail fast，也不得错误跳到另一个 owner 的 frame layout。
  4. 补充定向测试覆盖至少：
     - `effect_multi_escape_custom_nonresuming_direct_indirect_multi.scoop`；
     - 一个同时拥有两个 owner function、相同 resume tuple / answer / wrapper schema、但不同 owner-local completion payload source 的最小单元或 fixture。

- 必须遵从的约束：
  - 禁止选择第一个 owner、按 object id 排序取一个、或把多 owner schema 拆成后端私有特例；
  - 禁止把失败 fixture 降级为显式 legacy、跳过、或改弱 golden；
  - 不得重新设计 Step / continuation ABI，只能补齐 P5/P6 handoff 中缺失的 owner-aware dispatch/projection 信息。

- 验证：
  - `cargo run -p scoop -- test --fixtures tests/fixtures/run-pass/effect_multi_escape_custom_nonresuming_direct_indirect_multi.scoop`
  - `cargo run -p scoop -- test --fixtures tests/fixtures/run-pass/effect_multi_escape_custom_nonresuming_direct_indirect_block_multi.scoop`
  - `cargo run -p scoop -- test --fixtures tests/fixtures/run-pass/effect_escape_continuation_indirect_perform_multi_site_callee_branch.scoop`
  - 相关 `scoopc` effect-lowered / LLVM ABI materialization 单元测试。

- 完成条件：
  - 多 owner continuation object 共享 owner-trampoline surface resume schema 时，ABI materialization 和 runtime resume 都选择正确 owner；
  - 上述 multi-function direct/indirect continuation fixtures 在默认 refactor run-pass 下通过；
...
## [DONE] P7-T02Z：闭合 P7-T03 剩余默认 run-pass refactor 阻塞，避免 full regression 依赖 legacy 或 fixture 降级
...
- 完成记录：
  ...
  - 2026-05-09：本轮复验确认 `P7-T02Z` 已满足完成条件，未再发现需要新增的前置 blocker。验证通过：`cargo run -p scoop -- test --fixtures tests/fixtures/run-pass/effect_resume_finally_body_raise_after_resume.scoop`；`cargo run -p scoop -- test --fixtures tests/fixtures/run-pass`（`fixtures: ok (391)`）；`cargo test --all`；`cargo run -p scoop -- test`（`fixtures: ok (1270)`）；`cargo clippy --all-targets -- -D warnings`。因此 `P7-T03` 可以回到标准 full-regression 矩阵收口。

## [DONE] P7-T03：在 refactor 成为默认主线后运行标准 full regression 矩阵，并修复所有默认路径回归
...
- 完成记录：
  ...
  - 2026-05-09：完成 `P7-T03` 最终验收。本轮先按标准矩阵顺序执行 `cargo test --all`、`cargo run -p scoop -- test`、`cargo run -p scoop_tools -- spec-fixtures check`、`cargo clippy --all-targets -- -D warnings`。其中第一次失败的是 `cargo run -p scoop -- test`：`tests/fixtures/run-pass/delegated_property_lazy_thread_safety_synchronized_once.scoop` 出现一次性 `run-pass` 3000ms 超时。失败根因类别经复验判断为非稳定回归：定向 `cargo run -p scoop -- run tests/fixtures/run-pass/delegated_property_lazy_thread_safety_synchronized_once.scoop`（default / explicit legacy）均输出期望结果；隔离执行 `target/debug/scoop test --fixtures tests/fixtures/run-pass/delegated_property_lazy_thread_safety_synchronized_once.scoop` 五次均通过，耗时约 1.47s-1.72s；随后重跑完整 `cargo run -p scoop -- test` 也通过（`fixtures: ok (1270)`）。在此基础上再整套重跑标准矩阵：`cargo test --all`、`cargo run -p scoop -- test`、`cargo run -p scoop_tools -- spec-fixtures check`、`cargo clippy --all-targets -- -D warnings` 全部通过。额外 compare smoke 也通过：`cargo run -p scoop -- --effect-pipeline legacy test --fixtures tests/fixtures/run-pass/minimal_main.scoop`、`cargo run -p scoop -- --effect-pipeline legacy build --emit-llvm tests/fixtures/build/emit_llvm_basic.scoop -o /tmp/p7_post_regression_legacy.ll`。据此确认 omission/default=refactor 的标准 full regression 已可稳定收口，legacy 仍仅保留为显式 compare/rollback 入口。

## [DONE] P7-T03R：Review 标准 full regression，确认新默认主线已经覆盖常规回归而不是靠 legacy 兜底
...
- 依赖：P7-T03
- 完成记录：
  - 2026-05-09：完成 `P7-T03` 最终 review。复跑 `cargo test --all`、`cargo run -p scoop -- test`、`cargo run -p scoop_tools -- spec-fixtures check`、`cargo clippy --all-targets -- -D warnings`，均在 omission/default=`refactor` 条件下通过；同时复跑 explicit legacy smoke：`cargo run -p scoop -- --effect-pipeline legacy test --fixtures tests/fixtures/run-pass/minimal_main.scoop` 与 `cargo run -p scoop -- --effect-pipeline legacy build --emit-llvm tests/fixtures/build/emit_llvm_basic.scoop -o /tmp/p7_post_regression_legacy.ll`，确认 legacy 仍仅作为显式 compare/rollback 入口存在。
...
## [DONE] P7-T04R：Review P7 阶段退出条件，确认默认主线已切换且 P8 只需删除旧主线并再次 full regression
...
- 依赖：P7-T04
- 完成记录：
  - 2026-05-09：完成 `P7-T04R` 阶段退出 review。代码/文档复核结论如下：`crates/scoopc/src/session/mod.rs` 仍以 `EffectPipelineMode::Refactor` 作为 `Default`，因此 `Session::new()` / `SessionOptions::default()` / `Session::with_sysroot(...)` 在 omission 情况下统一进入 refactor；`crates/scoop/src/cli.rs` 与 `crates/scoopc/src/driver_cli.rs` 继续把 CLI omission 默认值固定为 refactor，并保留显式 `legacy` 仅作短期 compare/rollback；`crates/scoop/src/commands/mod.rs` 继续统一从 CLI selector 构造 `SessionOptions`；`crates/scoop/src/fixtures/mod.rs` 与 `crates/scoop/src/fixtures/run_pass.rs` 仍只在显式 legacy session 下向子进程追加 `--effect-pipeline legacy`，默认与显式 refactor 都通过 omission 传播；`crates/scoopc/src/effect_refactor_pipeline/mod.rs` 顶层注释与 `EFFECT_REFACTOR.md` §5.6.6 一致冻结了 P7 -> P8 handoff：默认主线已是 refactor，legacy 仅剩短期 compare/rollback，P8 只删除 selector/旧主线并再次 full regression。
  - 搜索/审计结论：对实现代码执行 `Legacy|Refactor|effect[-_]pipeline|Session::new\(|default.*legacy|default.*refactor` 与 `--effect-pipeline refactor|--effect-pipeline legacy|fallback.*legacy|retry.*legacy|default.*legacy` 等价搜索后，命中集中在 `crates/scoop/src/fixtures/mod.rs`、`crates/scoop/src/fixtures/run_pass.rs`、`crates/scoopc/src/driver_cli.rs`、`crates/scoop/src/commands/build.rs` 中的显式 compare/rollback 入口、legacy unsupported 诊断和 anti-fallback 断言；未发现 omission/default 回 legacy、refactor 失败后 retry/fallback legacy，或 helper/fixture hidden fallback。`TODO-P8.md` 的 `P8-T01` / `P8-T02` 也与当前 handoff 完整对齐：后续只需删除顶层 selector/dispatcher 与 legacy lowering/backend 本体，不需要重新讨论 selector、ABI、LLVM、GC/runtime 语义。
  - 复验通过：`cargo fmt --all`；`cargo test -p scoop --no-default-features cli`；`cargo test -p scoopc --no-default-features session`；`cargo test -p scoopc --no-default-features driver_cli`；`cargo test -p scoop --no-default-features default_refactor_pipeline`；`cargo test -p scoop --no-default-features explicit_legacy_pipeline`；`cargo test -p scoop --no-default-features dump_effect`；`cargo test -p scoop --test p7_default_pipeline`；`cargo test -p scoop legacy_frames`；`cargo test -p scoop no_hidden_legacy_fallback`；`cargo run -p scoop -- dump-ast tests/fixtures/parse/hello.scoop`；`cargo run -p scoop -- --effect-pipeline legacy dump-ast tests/fixtures/parse/hello.scoop`；`cargo run -p scoop -- dump-mir tests/fixtures/mir/handle_perform.scoop`；`cargo run -p scoop -- --effect-pipeline refactor dump-mir tests/fixtures/mir/handle_perform.scoop`；`cargo run -p scoop -- --effect-pipeline legacy dump-mir tests/fixtures/mir/handle_perform.scoop`；`cargo run -p scoop -- build --emit-llvm tests/fixtures/build/emit_llvm_basic.scoop -o /tmp/p7_default.ll`；`cargo run -p scoop -- --effect-pipeline legacy build --emit-llvm tests/fixtures/build/emit_llvm_basic.scoop -o /tmp/p7_legacy.ll`；`cargo run -p scoop -- --effect-pipeline refactor build --emit-llvm tests/fixtures/build/emit_llvm_basic.scoop -o /tmp/p7_explicit_refactor_build.ll`；`cargo run -p scoop -- test --fixtures tests/fixtures/build/emit_llvm_basic.scoop`；`cargo run -p scoop -- --effect-pipeline refactor test --fixtures tests/fixtures/build/emit_llvm_basic.scoop`；`cargo run -p scoop -- --effect-pipeline legacy test --fixtures tests/fixtures/build/emit_llvm_basic.scoop`；`cargo test --all`；`cargo run -p scoop -- test`；`cargo run -p scoop_tools -- spec-fixtures check`；`cargo clippy --all-targets -- -D warnings`；`cargo test -p scoopc --lib cross_call_escape_resume_roots_do_not_degrade_to_poison_in_explicit_frame -- --nocapture`；`SCOOP_GC_MOVE=1 SCOOP_GC_STRESS=1 SCOOP_GC_VERIFY_ROOTS=1 cargo run -p scoop -- run tests/fixtures/run-pass/effect_multi_escape_custom_nonresuming_direct_indirect_block_multi.scoop`；`SCOOP_GC_MOVE=1 SCOOP_GC_STRESS=1 SCOOP_GC_VERIFY_ROOTS=1 cargo run -p scoop -- run tests/fixtures/run-pass/continuation_escape_binder_resume_effect_row_runtime_basic.scoop`；`SCOOP_GC_MOVE=1 SCOOP_GC_STRESS=1 SCOOP_GC_VERIFY_ROOTS=1 cargo run -p scoop -- run tests/fixtures/run-pass/effect_escape_continuation_indirect_perform_binder_string_use.scoop`；`cargo run -p scoop -- run tests/fixtures/run-pass/effect_escape_continuation_arm_nested_handle_replay_tail_basic.scoop`；`SCOOP_GC_MOVE=1 SCOOP_GC_STRESS=1 SCOOP_GC_VERIFY_ROOTS=1 cargo run -p scoop -- test --fixtures tests/fixtures/run-pass`；`SCOOP_GC_MOVE=1 SCOOP_GC_STRESS=1 SCOOP_GC_VERIFY_ROOTS=1 cargo run -p scoop -- test --fixtures tests/fixtures/runtime_gc`；`cargo run -p scoop -- build --emit-llvm tests/fixtures/build/emit_llvm_basic.scoop -o /tmp/p7_final_default.ll`；`cargo run -p scoop -- run tests/fixtures/run-pass/minimal_main.scoop`；`cargo run -p scoop -- test --fixtures tests/fixtures/run-pass/minimal_main.scoop`；`cargo run -p scoop -- --effect-pipeline legacy test --fixtures tests/fixtures/run-pass/minimal_main.scoop`；`cargo run -p scoop -- --effect-pipeline legacy test --fixtures tests/fixtures/runtime_gc/gc_handle_roundtrip.scoop`；`cargo run -p scoop -- --effect-pipeline legacy build --emit-llvm tests/fixtures/build/emit_llvm_basic.scoop -o /tmp/p7_post_regression_legacy.ll`。
  - 结论：P7 已完成“切换主线并执行 full regression”的阶段目标；默认 CLI/session/fixture/main regression/GC env 均稳定指向 refactor，legacy 只剩显式 compare/rollback 入口。P8 可直接进入删除顶层 selector/dispatcher、legacy lowering/backend 与 compare-only 残留，再在单一 refactor 主线下重跑完整回归，无需重新打开任何 effect/continuation 设计讨论。
