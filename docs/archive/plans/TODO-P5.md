...
## [DONE] P5-T08：让 `NoOutward` 在 late-lowered handoff 中保持 plain callable，不物化 `Step` / continuation / state-machine 壳
...
- 依赖：P4-T06，P5-T07b
- 完成记录：
  - 2026-05-05：完成 `P5-T08`。`crates/scoopc/src/effect_lowered/ir.rs` 现已把 `LateLoweredCallable` 明确分为 `LateLoweredCallableAbi::Plain` 与 `LateLoweredCallableAbi::EffectStep`：Plain 分支只发布普通函数签名、canonical source slices 与 source-slice call-site ABI contracts；EffectStep 分支继续独占 `Step_F`、dynamic invoke、state graph、frame schema、boundary map、resume map、continuation object 与 resume packing handoff。
  - 2026-05-05：`crates/scoopc/src/effect_lowered/builder.rs` 现直接消费 P4 的 `CallableAbiKind`。对 `CallableAbiKind::Plain` / `ImplPlan::NoOutward` callable，builder 不再调用 whole-function segmentation、frame lifting、step/resume materialization、continuation object materialization 或 dynamic-invoke materialization；对 `EffectStep` callable 继续走既有统一 state-machine transformation。Plain body 中的 call site 现在通过 `LateLoweredPlainCallSite` 显式发布 `callee_abi_kind`、callee step schema、resolved cases 与 dispatch kind，使 plain direct/fun-value/interface call 可以保持 ordinary dispatch，同时 effect-step callee call 仍有 source-slice dispatch contract 可供 P6 消费。
  - 2026-05-05：`crates/scoopc/src/effect_lowered/dump.rs`、`opt.rs` 与 `crates/scoopc/src/effect_refactor_pipeline/effect_lowering_stage.rs` 已同步 ABI 分支：plain callable dump 显示 `abi: Plain`、`ordinary_signature`、`plain_source_slices`、`plain_call_sites` 与 `effect_step_handoff: <none>`，不再显示 body `Step_F`、continuation object、state graph、frame、boundary 或 resume map；post-lowering opt pass 会跳过 plain callable，只优化 EffectStep callable 的 state-machine/continuation shells。
  - 2026-05-05：已刷新 `tests/fixtures/effect_lowered/*.effectlowered` golden。`direct_and_fun_value_call` 现在显示 `step_type_count: 0`、`continuation_object_count: 0`，所有 `NoOutward` callable 均为 `abi: Plain`；含 effectful callable 的 fixture 只为真实 EffectStep callable 保留 `Step_F` / continuation / state graph handoff。
  - 新增/更新测试覆盖：`refactor_effect_lowered_no_outward_plain_callable_handoff`、`refactor_late_lowered_ir_plain_callable_has_no_step_shell_for_no_outward_body`、`refactor_source_slice_plain_call_keeps_ordinary_call_contract_without_boundary_dispatch`，并调整既有 stage/materialize/segmentation 测试以断言 `NoOutward` 不再是 entry+complete 伪状态机。
  - 验证通过：`cargo test -p scoopc --no-default-features refactor_effect_lowered_no_outward_plain_callable`、`cargo test -p scoopc --no-default-features refactor_late_lowered_ir`、`cargo test -p scoopc --no-default-features refactor_source_slice_plain_call_keeps_ordinary_call_contract`、`cargo test -p scoopc --no-default-features refactor_effect_lowered_stage`、`cargo test -p scoopc --no-default-features refactor_continuation_object`、`cargo test -p scoopc --no-default-features refactor_resume_interface_completeness`、`cargo test -p scoopc --no-default-features refactor_impl_plan_lowering_keeps_no_outward`、`cargo run -p scoop --no-default-features -- --effect-pipeline refactor dump-effect-lowered tests/fixtures/effect_lowered/direct_and_fun_value_call.scoop`、`cargo run -p scoop --no-default-features -- test --effect-pipeline refactor --fixtures tests/fixtures/effect_lowered`、`cargo clippy -p scoopc -p scoop --no-default-features --all-targets -- -D warnings`。`PLAN.md` 无需改动。
