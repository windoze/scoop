...
- 进展更新（2026-05-01）：`T5001f8a0` / `T5001f8a0R` 已完成。已复核 state/arm body 对 outer mutable local 的读写入口会在 use-site 通过 `rematerialize_ptr_in_current_block(...)` 重建指针链，避免依赖 stale heap-slot pointer；定向验证 `continuation_resume_enum.scoop`（GC env）与 runtime_gc fixture `effect_outer_mutable_state_body_writeback_basic.scoop` 均通过。
- 进展更新（2026-05-01）：`T5001f8a` 已完成。已在默认环境与 `SCOOP_GC_MOVE=1 SCOOP_GC_STRESS=1 SCOOP_GC_VERIFY_ROOTS=1` 下复验 `continuation_resume_enum.scoop`，并确认最小回归 `effect_escape_continuation_outer_mutable_writeback_basic.scoop` 覆盖“handle-return 后立刻读回”的窗口；同时 `cargo test --all` 与 `cargo clippy --all-targets -- -D warnings` 均通过。
- Review 状态（T5001f8aR，2026-05-01）：已复核 `write_back_outer_scope_frame_slots(...)` 与 caller-side handle 退出路径，确认 handle-return 仍位于原 caller activation 时会优先经 caller 当前稳定 local home/backing slot 写回，从而同步 caller explicit-frame home slots；新增最小 fixture 在默认环境与三项 GC env 全开条件下均稳定通过，caller 紧随 handle 之后读取不再落回旧镜像。
- 进展更新（2026-05-01）：`T5001f8b` 已完成。state-machine 侧不再把 heap frame field GEP 作为 env local home：为每个 frame-backed local materialize entry alloca 作为稳定执行期 local home，并在每个 state BB 入口从 heap frame backing slot reload 到该 home；对 local assignment、`BindLocal/ReadLocal`、arm binder/capture 与 escape continuation binder 写入路径统一做 write-through（exec home + backing heap slot）。新增 LLVM 回归 `state_machine_frame_slots_materialize_stable_exec_local_homes` 锁定该 contract；并已通过 `cargo test --all`、`cargo clippy --all-targets -- -D warnings` 与定向 run-pass fixture 复核。
- Review 状态（T5001f8bR，2026-05-01）：已复核 state-machine 的 handle body locals、arm binder、capture locals 与 escape-continuation binder 绑定路径，确认 env `CgLocal.ptr` 统一收口到 entry alloca（`handle_frame_home_*`），heap frame slot GEP 仅作为 `frame_backing_ptr` 承载持久化 write-through，不再长期暴露为执行期 local home；对应 LLVM 回归已能锁定 materialize/使用 `handle_frame_home_saved` 的 contract。
