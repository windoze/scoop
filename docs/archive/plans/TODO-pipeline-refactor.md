...
> 当前状态：P0-P8、`TODO-7-INIT`、P9 全部任务、`P10-T01`、`P10-T01R`、`P10-T02`、`P10-T02R`、`P10-T03-a`、`P10-T03`、`P10-T03R`、`P10-T04-a`、`P10-T04`、`P10-T04-b`、`P10-T05`、`P10-T05R`、`P10-T06`、`P10-T06R`、`P10-T04-c-1`、`P10-T04-c-2` 与 `P10-T04-c-3` 已完成；`P10-T04R` review 发现 cache-hit dependency cone 在下游 stage 不可见的阻塞性缺陷，由 `P10-T04-b`（frontend/effect_facts/mir 中端注入路径，已完成）与 `P10-T04-c`（LLVM codegen 层 dep `LateLoweredProgram` / `.o` artifact handoff）共同覆盖；**`P10-T04-c` 在 2026-05-25 自治执行调研后被拆分为 4 个子任务**（`P10-T04-c-1..-4`，详见 `TODO-7.md`）——`P10-T04-c-1` 已把 cross-program ABI mangling 的 program-local `body#<index>` 替换为 content-stable LIR callable key，并让 consumer pipeline 与 dep 子进程对同一 dep callable 产出一致 symbol；`P10-T04-c-2` 已把 cache-hit dep artifact 中的 `LateLoweredProgram` / `LirFacts` / `TypeStore` handoff 接入 LLVM ABI materializer，使 dep callable layout 查询不再依赖 dep AST 出现在 consumer program；`P10-T04-c-3` 已让 consumer cache-hit 直接复用 dep artifact 并剔除 dep AST，中端通过 cached frontend import 与 cached dep LIR handoff 继续 lower consumer，fresh 子进程下剩余 dep `.o` link 接管由 `P10-T04-c-4` 覆盖；为支持 per-cone 多进程并发编译，新增 `P10-T05` / `P10-T05R`（CLI 参数与并发抽象）、`P10-T06` / `P10-T06R`（cone DAG 子进程并发 driver），原 P10 全包清场任务顺延为 `P10-T07` / `P10-T07R`；`P10-T05` 已落地 CLI `-j/--jobs` + `SCOOP_BUILD_JOBS` env + `ConcurrencyStrategy` / `SubprocessConeCompiler` trait surface（不引入并发执行行为）；`P10-T05R` review 通过；`P10-T06` 已落地：scoopc 新增 `build-single-cone` 子命令、`scoop` driver 新增 `commands/build/scheduler.rs` 状态机调度器（Pending/InFlight/Done/Failed + cache-hit 短路 + 失败传播 + cone-prefixed diagnostic）、`LocalProcessConeCompiler` 真正 fork+exec scoopc 子进程派发 LocalDependency cone、6 个 driver 单测（包含 `RecordingFakeConeCompiler` 与 `BarrierFakeConeCompiler` 真正验证 trait 解耦 + 并发上限 + 串行回退）、端到端 `cargo run -p scoop -- test` 1507/1507 fixture 全部通过、`source_path_dependency_public_call` 在 `--jobs 1` 与 `--jobs 4` 上 fresh build 与 cache hit 等价产出；`P10-T06R` review 通过：cone DAG 拓扑/失败传播/trait 抽象/jobs=1 与并发覆盖/scoopc 单 cone 边界/cache-hit 与子进程混合下游注入 6 项检查均通过，无阻塞性缺陷；**2026-05-25 facade 边界整顿任务追加**：在 `P10-T06R` 与 `P10-T07` 之间插入 `P10-T06-b` / `P10-T06-c`，分别覆盖 (1) 抽出 `scoopld` crate（link + runtime C 编译）、scoopc 增 `link-cone` 子命令、收紧 `build-single-cone` 上游边界（仅 sysroot auto-dep 4 cone 可走源码）、独立 link fingerprint cache、kind dispatch API surface 就位；(2) consumer / virtual cone 全部走子进程派发、迁移 `prepare_virtual_cone_*` 到 scoop、加 `--sysroot-dep` CLI、删除 `CompilerCli::Legacy`、native build 进入 cone artifact、`scoopc_cone` 拆分（metadata 下沉到 project_model）、`scoop_project_model` 重命名为 `scoop_project_model`、scoop crate 仅保留 `scoop_project_model` 一项内部 crate 依赖（dependency_gate 强制）；执行顺序：`P10-T05` ✅ → `P10-T05R` ✅ → `P10-T06` ✅ → `P10-T06R` ✅ → `P10-T04-c-1` ✅ → `P10-T04-c-2` ✅ → `P10-T04-c-3` ✅ → `P10-T04-c-4` → `P10-T04R` → `P10-T06-b` → `P10-T06-c` → `P10-T07` → `P10-T07R`；下一项为 `P10-T04-c-4`。
...
> 2026-05-26 更新：`P10-T07R` 已完成；review 补回 scheduler 的混合 cache-hit、失败传播、并发上限与 jobs=1 串行回归覆盖，复核 `scoop` facade 依赖/source residual、P10 文档基线与完整验证均通过。P10 任务包已全部完成。
...
| P9-T07 | [DONE] | [`TODO-7.md`](./TODO-7.md#done-p9-t07cone-两层拆分scoop_project_model-扩展--新-scoopc_cone) | cone 两层拆分（`scoop_project_model` 扩展 + 新 `scoopc_cone`） |
...
| P10-T04-c | [DONE] | [`TODO-7.md`](./TODO-7.md#done-p10-t04-c让-cached-dependency-cone-在-llvm-codegen-层-callable_layoutslateloweredprogram-路径上可见) | （已由 c-1/c-2/c-3/c-4 收口，参见 TODO-7.md）让 cached dependency cone 在 LLVM codegen 层可见 |
| P10-T04-c-1 | [DONE] | [`TODO-7.md`](./TODO-7.md#done-p10-t04-c-1把-stable_lir_callable_key-的-bodyindex-后缀替换为-content-stable-canonical) | 把 `stable_lir_callable_key` 的 `body#<index>` 后缀替换为 content-stable canonical |
...
| P10-T07R | [DONE] | [`TODO-7.md`](./TODO-7.md#done-p10-t07rreview-p10-全包完成度) | Review P10 全包完成度 |
