...
- 依赖：P1-T03
- 完成记录：
  - 2026-05-02：完成 `P1-T03R` review，未发现需要在进入 `P2` 前补入的新前置缺陷；最近一次提交 `[P1-T03] Freeze AST handoff parity` 也未显式留下与本 review 直接相关的未完成事项。
  - AST stage 复核结论：`crates/scoopc/src/effect_refactor_pipeline/ast_stage.rs` 已把 AST -> typed handoff contract 固化到代码注释中，明确限制 P1 只保留普通 `Call` / `MemberAccess` surface 形状，不做 type-dependent desugar，并把 `k.resume()` / `k.resume(())`、`f()` / `f(())` 的等价性留给 P2 typed 阶段解释。
  - 路由复核结论：`crates/scoopc/src/effect_refactor_pipeline/mod.rs` 与 `refactor.rs` 继续保证 refactor AST stage 作为独立阶段入口存在；`crates/scoop/src/commands/dump_ast.rs` 的生产路径统一经 `load_ast_stage_output_for_dump(...)` 进入 CLI -> session -> dispatcher -> AST stage，不再整条命令偷跳 legacy。
  - surface contract 复核结论：`crates/scoopc/src/parser/tests.rs` 中 `parse_resume_member_call_as_plain_call_shape`、`parse_zero_arg_resume_member_call_without_unit_desugar`、`parse_zero_arg_and_explicit_unit_calls_as_distinct_shapes` 仍直接锁定 `k.resume(...)`、`k.resume()` 与 `f()` / `f(())` 的 AST 形状；`crates/scoopc/src/parser/expr.rs` 中 `peek_ident_text("resume")` 分支仅用于已删除 `-> resume { ... }` 旧语法的迁移诊断，不参与普通 member-call 解析。
  - parity 复核结论：`crates/scoop/src/commands/parity.rs` 的 `refactor_ast_stage_parity_*` 测试继续通过 CLI 参数显式切换 `legacy` / `refactor`，比较同一 parse fixture 的 `dump-ast` 输出一致，说明 P1 只冻结 contract 而未改变 AST 可观察语义。
  - 搜索摘要：对 `crates/scoopc/src/parser` 执行 `EffectPipelineMode|effect_pipeline|legacy|refactor` 搜索为 0 命中，说明 parser 业务代码仍未渗入 pipeline selector。
  - 复验通过：`cargo test -p scoopc --no-default-features ast_stage`、`cargo test -p scoopc --no-default-features effect_refactor_pipeline`、`cargo test -p scoop --no-default-features dump_ast_command_uses_refactor_ast_dispatcher`、`cargo test -p scoopc --no-default-features parser::tests`、`cargo test -p scoop --no-default-features refactor_ast_stage_parity`、`cargo run -q -p scoop --no-default-features -- --effect-pipeline legacy test --fixtures tests/fixtures/parse/continuation_resume_member_call_basic.scoop`、`cargo run -q -p scoop --no-default-features -- --effect-pipeline refactor test --fixtures tests/fixtures/parse/continuation_resume_member_call_basic.scoop`、`cargo run -q -p scoop --no-default-features -- --effect-pipeline legacy test --fixtures tests/fixtures/parse/continuation_resume_unit_call_basic.scoop`、`cargo run -q -p scoop --no-default-features -- --effect-pipeline refactor test --fixtures tests/fixtures/parse/continuation_resume_unit_call_basic.scoop`、`cargo run -q -p scoop --no-default-features -- --effect-pipeline legacy test --fixtures tests/fixtures/parse/unit_single_param_zero_arg_call_basic.scoop`、`cargo run -q -p scoop --no-default-features -- --effect-pipeline refactor test --fixtures tests/fixtures/parse/unit_single_param_zero_arg_call_basic.scoop`、`diff -u <(cargo run -q -p scoop --no-default-features -- --effect-pipeline legacy dump-ast tests/fixtures/parse/hello.scoop) <(cargo run -q -p scoop --no-default-features -- --effect-pipeline refactor dump-ast tests/fixtures/parse/hello.scoop)`、`diff -u <(cargo run -q -p scoop --no-default-features -- --effect-pipeline legacy dump-ast tests/fixtures/parse/continuation_resume_member_call_basic.scoop) <(cargo run -q -p scoop --no-default-features -- --effect-pipeline refactor dump-ast tests/fixtures/parse/continuation_resume_member_call_basic.scoop)`、`diff -u <(cargo run -q -p scoop --no-default-features -- --effect-pipeline legacy dump-ast tests/fixtures/parse/unit_single_param_zero_arg_call_basic.scoop) <(cargo run -q -p scoop --no-default-features -- --effect-pipeline refactor dump-ast tests/fixtures/parse/unit_single_param_zero_arg_call_basic.scoop)`、`cargo clippy -p scoop -p scoopc --all-targets --no-default-features -- -D warnings`。
