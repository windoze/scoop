...
## 3. Cone 与 package 的语言边界

Cone 是 Scoop 的分发与构建单元，类似 Rust crate；`.cone` 是该单元的二进制归档格式。`package` 是 Cone 内的源码级命名空间，类似 Rust module；源文件继续通过 `package foo.bar` 声明自己所在的命名空间。

语言层面需要固定以下内容：

- 一个 Cone 是编译、可见性、依赖解析和产物归档的边界。
- `public` 符号可跨 Cone 使用并进入公共 API。
- `internal` 符号只在同一 Cone 内可见。
- `private` 顶层符号只在同一源文件内可见。
- 源文件通过 `package` 与 `import` 构成 Cone 内可解析的源码图。
- 可执行 Cone 的入口由程序边界规则确定，通常是 `main` 函数；常规约定入口文件可命名为 `src/main.scoop`，但语言入口不是由文件名本身决定。

`.cone` 的内部归档结构、IR 版本、预单态化缓存、二进制分发和构建系统策略是工具链契约，不属于本文的语言语义。`Cone.toml` 中的平台源选择是包管理/构建层能力；它不得向语言引入预处理器语义。
...
- 插值表达式按普通表达式规则检查和求值。
- 普通字符串字面量与未带 `f` 前缀的 raw 字符串字面量是编译期可知的不可变 `String` 值；实现可将相同内容池化为只读 immortal `String`，不要求每次求值分配新对象。
