...
| `scoopc check-source --phase {parse,resolve,typecheck,infer} --input <file-or-cone-dir> [--source <path>] [--target-platform <id>]` | Empty. | Empty unless diagnostics are emitted. | Runs only the requested frontend phase. File inputs check that single source and reject `--source`; cone project inputs check every source in the source graph unless `--source` selects one project source path relative to the cone root or by absolute path. `--target-platform` applies to platform selectors and typecheck capability gates. |
