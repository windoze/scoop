...
## [DONE] P0-T02R：Review statement-level comptime 删除结果
...
- 依赖：P0-T02
- 完成记录：
  - 复查范围：逐项复查了 AST statement surface、statement parser、parser tests、const/comptime interpreter、typecheck statement/lowering、HIR statement lowering、MIR materialization templates，以及 P0-T02 删除后仍可能出现 statement-level comptime/splice 兼容逻辑的位置。
  - Review 结论：活跃源码中未发现 `StmtKind::Comptime*`、`ComptimeFor`、`RuntimeComptimePlan`、`parse_comptime_stmt`、`plan_comptime_*` 或 `ComptimeIf`/`ComptimeFor` AST/parser/runtime plan 残留；statement-level `comptime if/for` surface 和 runtime comptime plan 已物理删除，没有迁移成 typecheck/HIR/MIR 的新特判。
  - 修正项：清理了 `crates/scoopc/src/ast/mod.rs` 中 `splice_field_contracts` 注释对已删除 comptime 展开值的过时描述；HIR lowering 现在只消费 typecheck 发布的 splice field contract。
  - Fixture 结论：旧 HIR/MIR/runtime comptime 控制流与 splice fixtures 已不存在；`tests/fixtures/parse/comptime_syntax_basic.scoop` 保留为普通 parse-fail fixture。额外搜索到的 `comptime if` fixtures 只属于 P0-T01 的 package-level parse-fail 回归，不表示 statement-level surface 可用。
...
## [DONE] P0-T03R：Review `const` surface 与 evaluator 删除结果
...
- 依赖：P0-T03
- 完成记录：
  - 复查范围：逐项复查了 `syntax/token.rs`、`syntax/lexer.rs`、`parser/decls.rs`、`ast/mod.rs`、`resolve/mod.rs`、`session/mod.rs`、`hir/lower/types.rs`、`lib.rs`、实际 sysroot core 文件 `sysroot/lib/scoop.core/src/core.scoop`，以及 `tests/fixtures/umb_fix/B-24-reflection-comptime/` 空目录状态。
  - Review 结论：旧 Scoop `const` modifier、`const fun` / `const val` surface、`ConstEvalError` 包装、`is_const` 签名位和 `crates/scoopc/src/comptime/` evaluator 模块已从活跃源码、sysroot 与 fixtures 中删除；没有发现以其它名字复活的 const evaluator 或跨阶段 const-eval hooks。
  - 搜索结论：`pub mod comptime|ConstEval|Modifier::Const|Keyword::Const|\bis_const\b|const fun|const val` 于 `crates/`、`sysroot/`、`tests/fixtures/` 均无命中；额外 `comptime` 命中仅来自旧 package/statement parse-fail 回归、annotation retention policy 命名、HIR stage 人工 TODO reason 测试和 sysroot/overlay 中的 compile-time metadata 注释，不属于旧 `const fun` / `const val` surface 或 const evaluator。
  - Fixtures/sysroot 结论：B-24 reflection-comptime fixture 目录为空；主 sysroot 与 build sysroot overlay 中未发现 `const fun` / `const val` 声明，reflection helper 已保持普通运行期 intrinsic/API 形状。
