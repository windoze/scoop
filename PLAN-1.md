...
- 2026-04-07：完成 T1116：cone native build 支持 `cxx-sources/cxx-flags`：driver 在最终链接前编译 `Cone.toml[native-build].cxx-sources` 并参与链接；当存在 `cxx-sources` 且未指定 `native-build.linker` 时默认使用 `clang++` 做最终链接 driver，并通过 `-x c ... -x none` 强制 runtime C 源文件按 C 语言编译（避免 `.c` 被当作 C++ 导致编译错误）；新增 `tests/fixtures/run_pass_cone/cxx_sources_extern_call_cpp_stdlib_basic/**` 端到端回归；同时为 `gc_immix_allocator` Immix 集成测试增加进程内互斥锁，避免并发执行导致 `cargo test --all` 偶发卡死。验收：`cargo test --all` + `cargo run -p scoop --features llvm -- test` 通过。
- 2026-04-07：完成 T1117：toolchain 为 runtime sources 与 cone user `c-sources/cxx-sources` 注入 build profile/target 宏（`SCOOP_BUILD_PROFILE`/`SCOOP_DEBUG`/`SCOOP_TARGET_*`），并新增单测覆盖 link/compile 命令包含相应 `-D` 宏。验收：`cargo test --all` 通过。
- 2026-04-07：完成 T1118：driver 新增 `scoop new <project-name>` 创建最小 CONE application 项目骨架（`src/main.scoop` + `Cone.toml` + `README.md`），包含 project name 校验与目录存在拒绝，并新增 `crates/scoop` 单测回归。验收：`cargo test -p scoop` 通过。
- 2026-04-07：完成 T1220a：Parser/AST 支持 package 顶层 `comptime if`（分支块内为顶层 items 列表），并在语句级 `comptime if` 同步支持 `else if` 语法糖；新增 parse pass/fail fixtures 回归。验收：`cargo test --all` + `cargo run -p scoop -- test` 通过。
...
- 2026-04-07：完成 T1412e：为短窗口场景提供 `scoop_gc_try_collect_minor(deadline_ms)`：deadline 内无法达成协作式 STW 则撤销 STW 请求并唤醒已 park 线程；新增 `crates/scoop_runtime/tests/gc_immix_try_minor_deadline.rs` 集成测试回归“超时返回 + 后续 major collect 仍可执行”。验收：`cargo test -p scoop_runtime --no-default-features --features gc-immix -- --test-threads=1` + `cargo test --all` 通过。
...
- 2026-04-07：完成 T1516：落实 rich enum 的 GC ref slots 契约：typecheck/layout 为 tagged union enum/`Option<T>` 生成 `gc_ref_word_mask` 与 `ref_payload` 元数据；LLVM codegen 将 boxed enum variant payload 改为 GC-managed heap object（`scoop_alloc_typed`），并把 payload 指针写入 enum 的 GC slot（`payload_ptr`），保证 moving GC 下 stack/heap slots 可被静态枚举与原地更新；新增 `runtime_gc` fixture 覆盖 enum 在栈与 heap 字段中跨 moving GC 后仍可解构读取。验收：`cargo test --all` + `cargo run -p scoop -- test` 通过。
...
- 2026-04-07：维护：为保证 TODO 顺序“首个未完成任务可直接实现”，将 T1412（minor GC）子任务按依赖关系重排为 `T1412d → T1412c → T1412e`，并移除该组三条任务的 `[BLOCKED]` 标记；当前剩余未完成项为：无。
...
- 2026-04-06：完成 GC-FIX Phase C2c：移除 sysroot `print/println(Int)` 的 runtime 绕路符号（`scoop_print{,ln}_{i64,u64}`），改为 codegen 侧统一走“format → malloc → `scoop_alloc_typed` 分配 GC-managed String → `scoop_print{,ln}`”路径；新增 LLVM 单测 `println_int_lowers_via_string_formatting_without_print_int_helpers`；新增 run-pass fixture `println_string_statepoint_basic`；并更新 `gc_mark_sweep_basic`/`gc_trace_struct_string_field_basic` 的 object count golden（因为 println(Int) 现在会分配临时 String）。验收：`cargo test --all` + `LLVM_SYS_181_PREFIX=/opt/homebrew/opt/llvm@18 cargo test -p scoopc --features llvm` + `LLVM_SYS_181_PREFIX=/opt/homebrew/opt/llvm@18 cargo run -p scoop --features llvm -- test` 通过。
- 2026-04-06：完成 GC-FIX Phase C2d：补齐字符串/数组/closure capture/interface dispatch 的端到端回归面，并新增 `RUN-MODE: dump-stackmaps` fixtures（`tests/fixtures/run-pass/stackmaps_dump_*`）确保 `scoop dump-stackmaps` 在这些场景下可解析。验收：`cargo test --all` + `cargo run -p scoop --features llvm -- test` 通过。
- 2026-04-06：完成 GC-FIX Phase D1：移除 `crates/scoop_runtime/tests/shadow_stack.rs`；Rust tests 全面改走 `native_roots`/handles/pin，相关覆盖由 `gc_enter_native.rs`（InNative roots）+ `gc_immix_compaction.rs`（moving roots slot 写回）+ `stackmap_registry.rs`（registry/lookup smoke）等承担。
- 2026-04-06：完成 GC-FIX Phase D2：审计并确认 `crates/scoop_runtime/tests` 已彻底移除手工 `ScoopGcFrame` roots，统一改用 `enter_native` roots slots + stable handles + `pin/unpin`；验收：`cargo test --all` + `cargo test --all --no-default-features --features gc-immix` 通过。
...
- 2026-04-05：完成 GC-FIX Phase B2c：新增 env `SCOOP_GC_VERIFY_ROOTS=1` 强校验模式；GC 周期结束后（仍在 STW 内）重新枚举 roots slots（stackmap/native_roots/handles/pin），并对每个非 NULL roots 做“必须指向 live 集合”的一致性校验；发现异常打印诊断并 fail-fast（`abort()`）；新增 `crates/scoop_runtime/tests/gc_verify_roots.rs` smoke；验收：`cargo test --all` + `cargo test --all --no-default-features --features gc-immix` + `cargo run -p scoop -- test` 通过。
- 2026-04-05：完成 GC-FIX Phase B3：runtime API 清理：移除所有 shadow stack 导出符号（`ScoopGcFrame`、`scoop_gc_frame_push/pop`、`scoop_gc_shadow_stack_visit_roots_*`、`scoop_gc_debug_count_roots_current_thread`、`scoop_gc_current_frame`）与 sysroot 测试辅助接口 `__scoop_gc_debug_count_roots_current_thread`；同步更新 `runtime/c/scoop_runtime_api.h` allowlist；删除对应 run-pass fixture 与 `scoop_runtime` 集成测试。
- 2026-04-05：完成 GC-FIX Phase C1：LLVM codegen 删除 shadow stack 插桩：移除 `GcFrameState` 与 `scoop_gc_frame_push/pop` 等 frame/slot 维护逻辑；并更新 `scoopc` LLVM stackmap roots 契约单测，确保无 shadow stack 时仍能产出 roots locations；验收：`LLVM_SYS_181_PREFIX=/opt/homebrew/opt/llvm@18 cargo test -p scoopc --features llvm` + `cargo test --all` 通过。
...
- 2026-04-04：T1505（safepoint poll + 线程状态机 + STW 协议 + native 过渡 + stack walking ctx）范围较大。为保持 TODO 顺序“首个 `[TODO]` 可直接实现”，拆分为 T1505a/T1505b/T1505c：先落地 `scoop_gc_safepoint_poll` API + STW 状态机/epoch/超时诊断（T1505a），再补齐 park 前捕获 unwind 上下文（T1505b），最后补齐 `enter_native/leave_native` 与 `native_roots` buffer（T1505c）。多帧 stack walking roots 的端到端回归将随 T1411b/T1506 一并落地。
- 2026-04-04：完成 T1505a：新增 `scoop_gc_safepoint_poll` ABI（当前与 `scoop_gc_safepoint` 等价），baseline STW 对齐线程状态机（Running/Parked）与 `epoch/last_safepoint_epoch` 诊断字段，并在等待 park 时提供超时诊断；新增 `scoop_runtime` 多线程测试回归 poll 的 park/resume；同时修复 `stackmap_registry` 集成测试在并行执行下的 flaky（串行化全局 registry 相关测试）。
- 2026-04-04：完成 T1505b：`scoop_gc_safepoint_poll` slow path 在进入 Parked 前捕获 opaque stack walking ctx（POSIX v0 先用 `setjmp(jmp_buf)` 保存寄存器快照；Windows backend 维持占位）；并在 STW begin/end 防御式清空 ctx；新增 `scoop_runtime` 集成测试回归“park 期间 ctx 非空、STW 结束 ctx 清空”。
- 2026-04-04：完成 T1505c：新增 `scoop_enter_native/scoop_leave_native`，线程在进入/离开 native 过渡态时切换为 InNative/Running 并维护 TLS `native_roots`；STW 等待时将 InNative 视为已就绪，并在 GC roots 枚举（含 Immix moving roots update）中扫描/更新 `native_roots`；新增 `scoop_runtime` 集成测试回归“线程长期 InNative 时 GC 不死锁且 roots 可保活”。
- 2026-04-04：T1510（GC 与 `@Extern`/FFI：native 过渡协议）覆盖 stable handle、编译器插桩、fixtures 等多个能力点。为保持 TODO 顺序“首个 `[TODO]` 可直接实现”，拆分为 T1510a/T1510b/T1510c：先落地 runtime stable handle 表与 C ABI（T1510a），再把 handle 暴露为 sysroot intrinsics（T1510b），最后实现 `@Extern` 的 `enter_native/leave_native` + roots buffer（T1510c）。
- 2026-04-04：完成 T1510a：runtime 新增 stable handle C ABI（`scoop_handle_new/get/drop`），并在 baseline/minimal/hosted/immix backend 的 GC roots 枚举中扫描 handle 表；Immix compaction（moving）阶段额外更新 handle->obj 槽位；新增 `scoop_runtime` 集成测试 `stable_handle` 回归“handle 保活 + drop 后可回收”。
- 2026-04-04：完成 T1510b：sysroot 新增 `GcHandle` 与 `GC.handleNew/handleGet/handleDrop`；typecheck/LLVM codegen 对三者做 intrinsic lowering 并映射到 runtime `scoop_handle_new/get/drop`；新增 run-pass fixture `gc_handle_roundtrip` 回归（`cargo run -p scoop --features llvm -- test`）。
- 2026-04-04：完成 T1510c：LLVM codegen 对 `@Extern` 调用点自动插入 `scoop_enter_native(root_slots, len)` / `scoop_leave_native()`，并用“局部变量 alloca 槽位地址”作为保守 roots slots；新增 runtime test-only 符号 `scoop_test_gc_collect_in_native` 与 run-pass fixture `extern_enter_native_roots_gc` 回归“native 内触发 GC 仍能保活 call-site roots”。
...
- 2026-04-05：完成 T1512c：新增多线程 runtime_gc fixtures 覆盖“跨线程 STW roots 枚举（Parked）”与“线程长期 InNative（native_roots）”两类场景；并新增 runtime test-only extern 以构造可控的 native 阻塞窗口，回归验证 GC 不死锁且 roots 保活。
- 2026-04-05：完成 T1512d：新增 runtime_gc fixture `gc_pin_unpin_move_stress_matrix`，通过 `scoop test --gc-stress/--gc-move` 驱动多轮压力并回归 pin count + pinned 地址稳定/解绑后可搬迁；新增 test-only extern `scoop_test_handle_get_object_addr` 以 `UIntPtr` 观测 stable handle 当前指向对象地址；新增 `EXPECT: fail` 用例回归 run-pass stdout mismatch 的稳定诊断码（llvm on/off 均可覆盖）。
- 2026-04-05：完成 GC-FIX Phase A1：扩展 `scoopc::stackmap` v3 解析至 records/locations，并固化“roots locations 连续后缀（Direct/Indirect + pointer-sized + SP/FP base）且成对出现”的语义契约；`scoop dump-stackmaps` 新增 `--verify-roots`（失败给出 record/location 诊断）；新增 `scoopc --features llvm` 单测回归覆盖契约与“至少一个 record 含 roots”。
- 2026-04-05：完成 GC-FIX Phase A2：stackmap registry `return_address -> record` lookup 从“最近邻窗口匹配”升级为“无歧义命中”（仅允许确定性候选 `ra`/`ra±1`；相邻 records 不再误配）；同 key 冲突/多候选在 `SCOOP_STACKMAP_STRICT=1` 下 fail-fast；新增 `scoop_runtime` 集成测试覆盖“无窗口误命中/歧义拒绝/-1 归一化”并回归 `cargo run -p scoop --features llvm -- test`。
- 2026-04-05：完成 GC-FIX Phase A3：platform/unwind 为 GC stack walking 提供可验证的 `(sp/cfa, ra, fp)` 输入（POSIX 捕获时强制 CFA 单调不减；Windows v0 支持 x86_64 的 ctx capture + frame walk）；`SCOOP_STACKMAP_STRICT=1` 下禁止“从 CFA 猜 FP”；新增 test-only 帧校验输出 `scoop_test_unwind_dump_frames_and_stackmap_hits` 与 `scoop_runtime` 回归测试。
...
- 2026-04-04：完成 T1507a：runtime 侧 `Array<Ref/String>` 通过 type descriptor `trace_fn` 扫描元素；LLVM codegen 为 class/closure env/string/box 写入对象头 `type_desc`（trace bitmap + type_id），并新增 run-pass fixtures 回归 class field/`Array<String>` 在 `__scoop_gc_collect()` 后仍可用；本地验证 `cargo test --all`、`cargo test -p scoopc --features llvm`、`cargo run -p scoop --features llvm -- test` 通过。
- 2026-04-04：完成 T1507b：driver `scoop dump-rtti` 从编译器侧导出 type descriptor v0（`type_id`/`parent`/`parent_chain`/`trace_bitmap_u64`/`trace_fn`），并新增最小 parse fixture 作为手动验收用例；`cargo test --all` 通过。
- 2026-04-04：T1507c（interface id + vtable/itable）仍较大。为保持 TODO 顺序“首个 `[TODO]` 可直接实现”，进一步拆分为 T1507c1/T1507c2/T1507c3：先导出 interface id + method slots（T1507c1），再生成 class vtable slot layout（T1507c2），最后生成 class itable entries（T1507c3）。
- 2026-04-04：完成 T1507c1：为 interfaces 导出稳定 `interface_id` 与 `method_slots`（`scoop dump-rtti` JSON 可观测），并新增单测覆盖；本地验证 `cargo test --all`、`cargo run -p scoop -- test` 通过。
- 2026-04-04：完成 T1507c2：编译器侧为 class 生成稳定 vtable slot layout（继承 + `override` 覆盖），并在 `scoop dump-rtti` JSON 中导出 `vtable_slots`；新增 `scoopc` 单测覆盖 “override 复用 slot”；本地验证 `cargo test --all`、`cargo run -p scoop -- test` 通过。
- 2026-04-04：完成 T1507c3：编译器侧为 class 生成稳定 itable entries（按 `interface_id` 排序），并在 `scoop dump-rtti` JSON 中导出 `itable_entries`；新增 `scoopc` 单测覆盖“接口实现映射（含 class 继承 + interface 继承）”；本地验证 `cargo test --all`、`cargo run -p scoop_tools -- spec-fixtures check`、`cargo run -p scoop -- test` 通过。
- 2026-04-04：T1508（member call + 动态分发）涉及 typecheck + lowering + 最小 codegen 接入，跨度较大。为保持 TODO 顺序“首个 `[TODO]` 可直接实现”，拆分为 T1508a/T1508b/T1508c：先落地 direct member call（final/private/owner final）与 member fun 的 HIR 形态（T1508a），再补齐 vtable virtual dispatch（T1508b），最后补齐 itable interface dispatch（T1508c）。
- 2026-04-04：完成 T1508a：typecheck 支持对已解析 member fun 的 overload resolution（隐式注入 `this`/receiver 为第 0 实参）；HIR lowering 将 class/object 的 member call desugar 为顶层调用，并收集 member fun 供 LLVM codegen；新增 typecheck + run-pass fixtures 回归；本地验证 `cargo test --all`、`cargo run -p scoop -- test`、`cargo run -p scoop --features llvm -- test` 通过。
- 2026-04-04：完成 T1508b：编译器侧新增 class vtable slot layout side table（继承 + `override` 复用 slot），LLVM 后端生成 per-class vtable 常量并写入 `ScoopTypeDescriptor.vtable`，调用点按 slot 从 `this.header.type_desc.vtable[slot]` 取函数指针并 indirect call；新增 run-pass fixture 回归 `Base` 引用调用动态分发到 `Derived.override`；本地验证 `cargo test --all`、`cargo run -p scoop -- test`、`cargo run -p scoop --features llvm -- test` 通过。
- 2026-04-04：完成 T1508c：interface dispatch（itable）端到端落地：HIR lowering 对 interface member call 做 desugar；LLVM 后端为 class 生成 itable 常量并写入 `ScoopTypeDescriptor.itable`，调用点按 `interface_id + slot` 查找并 indirect call；新增 run-pass fixture `member_call_interface_dispatch_basic` 与 typecheck fixture `ambiguous_interface_member_impl` 回归覆盖；本地验证 `cargo test --all`、`cargo run -p scoop -- test`、`cargo test -p scoopc --features llvm`、`cargo run -p scoop --features llvm -- test` 通过。
- 2026-04-04：完成 T1509b：`is/!is/as/as?` HIR lowering 保留 `TypeCheck/Cast` 节点，LLVM codegen 落地 class parent 链与 interface itable 的运行期检查；`as?` 失败返回 `None`，`as` 失败触发 `Raise.raise(RuntimeError.ClassCastFailed)`；新增 run-pass fixture `type_check_cast_is_as_asq_basic.*` 回归，并本地验证 `cargo test --all` 与 `cargo run -p scoop --features llvm -- test` 通过。
...
- 2026-04-03：T1409c（并行标记/并行 sweep）范围较大。为保持 TODO 顺序“首个 `[TODO]` 可直接实现”，已拆分为 T1409c1（并行标记 v0）、T1409c2（并行 region sweep v0）、T1409c3（并行 mark/sweep stress 回归）。
- 2026-04-03：完成 T1409c1：Immix 并行标记 v0（可开关）：STW 阶段引入 marker workers + 全局 work stack，并把 `obj->mark` 与 line mark bitmap 的写入升级为原子操作；新增 `gc_immix_parallel_mark` 集成测试覆盖开关关闭/开启两种模式。
...
- 2026-04-03：完成 T1411c：为 non-resuming effect（含 `Raise.raise`）增加稳定诊断 hook：LLVM codegen 在 set-active 时写入 call-site line/col，runtime 侧记录到 TLS 并暴露 `scoop_effect_trace_*` 读取 API（可选 env：`SCOOP_EFFECT_CAPTURE_UNWIND=1` 采样 IPs）；新增 run-pass fixture `effect_raise_trace_hook_basic` 回归覆盖。
...
- 2026-04-03：完成 T1502b2：Array/MutableArray 的 `String` 元素通道切换为 `scoop_array_*_ref`（push/get/set），避免把 `addrspace(1)` String 指针编码为 `u64`；新增 IR 单测回归 `Array<String>`/`MutableArray<String>` 不再出现 `ptr_to_u64/u64_to_string`。
- 2026-04-03：完成 T1502c：typecheck 在 `@Extern` 函数声明处加入 ABI 门禁：receiver/参数/返回值必须是 GC-free 值类型（禁止直接透传 GC ref），并给出稳定错误码 `scoop::typecheck::extern_fun_signature_must_be_gc_free` 与修复建议（`GC.pin/unpin` + `Ptr<T>`/handle）；新增 typecheck fixture 回归，并更新现有 fixtures 中的 `@Extern` 示例避免直接使用 `String`。
