...
### [DONE] P11-T01：审查 `__scoop_stackmap_statepoint_smoke` / `__scoop_gc_debug_*` 实际使用方
...
- 改动范围：
  - 本任务只做审计与任务记录更新，未迁移 sysroot / compiler / runtime surface。
  - 审计范围覆盖 `tests/fixtures/**/*.scoop` 中 4 个 helper 的实际调用点、custom sysroot overlay 中的声明副本、`sysroot/core.scoop` 当前声明形态、runtime 实现，以及 `crates/scoopc/src/llvm/tests/abi.rs` 中 stackmap smoke 的非 fixture 使用方。
  - 最新提交 `998aca30 [P10-T03] Verify core thread dependency cleanup` 未声明与 P11-T01 直接相关的未完成问题。
- fixture 使用方：
  - `__scoop_gc_collect`：50 个 fixture source 文件直接调用，用于显式触发真实 GC，并验证 root 保活 / 更新、moving GC、STW、pin/handle、effect cleanup、class init cleanup、generic/value boxing 与 String statepoint 等行为：`tests/fixtures/build/explicit_root_frame_default_mode_no_stackmaps.scoop`、`tests/fixtures/run_pass_cone/managed_abi_string_gc/src/helper.scoop`、`tests/fixtures/run-pass/class_init_raise_cleanup_init_block_gc_basic.scoop`、`tests/fixtures/run-pass/class_init_raise_cleanup_property_init_gc_basic.scoop`、`tests/fixtures/run-pass/closure_env_composite_capture_basic.scoop`、`tests/fixtures/run-pass/effect_handle_return_from_function_any_boxing.scoop`、`tests/fixtures/run-pass/effect_raise_cleanup_gc_basic.scoop`、`tests/fixtures/run-pass/enum_payload_boxing_any_basic.scoop`、`tests/fixtures/run-pass/gc_array_class_elements_cross_function.scoop`、`tests/fixtures/run-pass/gc_continuation_escape_alloc_heavy_resume.scoop`、`tests/fixtures/run-pass/gc_continuation_multi_thread_concurrent_alloc_resume.scoop`、`tests/fixtures/run-pass/gc_cross_function_class_object_graph.scoop`、`tests/fixtures/run-pass/gc_cross_function_struct_with_ref_fields.scoop`、`tests/fixtures/run-pass/gc_deep_nested_struct_class_ref.scoop`、`tests/fixtures/run-pass/gc_enum_ref_variants_cross_function.scoop`、`tests/fixtures/run-pass/gc_mark_sweep_basic.scoop`、`tests/fixtures/run-pass/gc_pin_unpin_basic.scoop`、`tests/fixtures/run-pass/gc_short_lived_long_lived_interleave.scoop`、`tests/fixtures/run-pass/gc_trace_array_string_elements_basic.scoop`、`tests/fixtures/run-pass/gc_trace_class_ref_field_basic.scoop`、`tests/fixtures/run-pass/gc_trace_struct_string_field_basic.scoop`、`tests/fixtures/run-pass/generic_class_gc_multi_alloc.scoop`、`tests/fixtures/run-pass/generic_class_gc_nullable_ref.scoop`、`tests/fixtures/run-pass/generic_class_gc_ref_field.scoop`、`tests/fixtures/run-pass/generic_class_gc_specialized_type.scoop`、`tests/fixtures/run-pass/generic_class_gc_value_field.scoop`、`tests/fixtures/run-pass/lang_mutable_array_new_string_ref.scoop`、`tests/fixtures/run-pass/println_string_statepoint_basic.scoop`、`tests/fixtures/run-pass/sync_gc_release_task_like_object_basic.scoop`、`tests/fixtures/run-pass/value_boxing_tuple_struct_any_basic.scoop`、`tests/fixtures/runtime_gc/array_intrinsic_composite_copy_gc_move_basic.scoop`、`tests/fixtures/runtime_gc/array_intrinsic_ref_set_gc_move_basic.scoop`、`tests/fixtures/runtime_gc/gc_handle_roundtrip.scoop`、`tests/fixtures/runtime_gc/gc_handle_token_roundtrip_callback_basic.scoop`、`tests/fixtures/runtime_gc/gc_module_global_roots_move_basic.scoop`、`tests/fixtures/runtime_gc/gc_move_enum_maybe_ref_closure_capture_basic.scoop`、`tests/fixtures/runtime_gc/gc_move_enum_maybe_ref_local_switch_basic.scoop`、`tests/fixtures/runtime_gc/gc_move_enum_maybe_ref_struct_field_basic.scoop`、`tests/fixtures/runtime_gc/gc_move_enum_ref_slots_basic.scoop`、`tests/fixtures/runtime_gc/gc_move_ordinary_call_struct_arg_basic.scoop`、`tests/fixtures/runtime_gc/gc_move_stackmap_heap_fixup.scoop`、`tests/fixtures/runtime_gc/gc_move_stackmap_roots_update_multi_frame.scoop`、`tests/fixtures/runtime_gc/gc_pin_unpin_move_stress_matrix.scoop`、`tests/fixtures/runtime_gc/gc_stw_cross_thread_in_native_roots_basic.scoop`、`tests/fixtures/runtime_gc/gc_stw_cross_thread_roots_basic.scoop`、`tests/fixtures/runtime_gc/gc_trace_array_string_elements_basic.scoop`、`tests/fixtures/runtime_gc/gc_trace_closure_capture_string_basic.scoop`、`tests/fixtures/runtime_gc/gc_trace_cycle_two_nodes_basic.scoop`、`tests/fixtures/runtime_gc/gc_trace_interface_dispatch_string_field_basic.scoop`、`tests/fixtures/runtime_gc/gc_trace_value_struct_ref_field_basic.scoop`。
...
- 决策表：
  ...
  - `__scoop_gc_debug_alloc_garbage`：保留，P11-T02 迁入 `scoop.runtime.test`，但不能按 C ABI leaf 迁移；应保持 GC-aware managed boundary（例如 `@Extern(name = "scoop_gc_debug_alloc_garbage", abi = "scoop")`，或继续由 compiler 对 test-cone FQN 做显式 root-preserving lowering）。理由：runtime 实现调用 `scoop_alloc`，而 `scoop_alloc` 会执行 safepoint poll，且在 GC stress 下会分配前调用 `scoop_gc_collect()`；多个 fixture 在调用前后持有 live refs，C ABI / native leaf 会丢失当前 root preservation contract。
  - `__scoop_stackmap_statepoint_smoke`：保留为 test-only cone 中的显式 stackmap smoke helper，但不能按 C ABI leaf 迁移；P11-T02 应把 special-case FQN 从 `scoop.core.__scoop_stackmap_statepoint_smoke` 调整到 test cone，并保留“仅对当前 caller 恢复 `gc "statepoint-example"` + ordinary managed runtime call”的 lowering。理由：该 helper 的测试目标就是让调用点产生真实 statepoint/stackmap record；`runtime/c/scoop_test.c` 注释也明确不能走 `@Extern` native/leaf lowering，否则不会生成 statepoint。虽然没有 fixture source 直接调用，Rust LLVM tests 仍覆盖该行为，因此不删除。
- 核心决策：
  - P11-T01 原任务草案中“`__scoop_gc_debug_alloc_garbage` / `__scoop_stackmap_statepoint_smoke` 可转 C ABI”的方向被实际实现审计否定；这不是 scope narrowing，而是基于 runtime/codegen contract 修正 P11-T02 的迁移方式。
  - 所有 helper 都从 core 迁出；除 stackmap smoke 的 compiler special-case 外，不保留 core alias、shim 或自动 prelude 可见性。
- 验证结果：
  - `rg -l "__scoop_gc_collect\\s*\\(" tests/fixtures --glob "*.scoop" --glob "!**/*.sysroot/**" | wc -l` -> 50。
  - `rg -l "__scoop_gc_debug_alloc_garbage\\s*\\(" tests/fixtures --glob "*.scoop" --glob "!**/*.sysroot/**" | wc -l` -> 11。
  - `rg -l "__scoop_gc_debug_heap_object_count\\s*\\(" tests/fixtures --glob "*.scoop" --glob "!**/*.sysroot/**" | wc -l` -> 8。
  - `rg -l "__scoop_stackmap_statepoint_smoke\\s*\\(" tests/fixtures --glob "*.scoop" --glob "!**/*.sysroot/**" | wc -l` -> 0。
  - `rg -l "__scoop_gc_collect|__scoop_gc_debug_alloc_garbage|__scoop_gc_debug_heap_object_count|__scoop_stackmap_statepoint_smoke" tests/fixtures/build --glob "**/*.sysroot/*.scoop"` -> 4 declaration-only overlay files。
  - `rg -n "__scoop_stackmap_statepoint_smoke|scoop_stackmap_statepoint_smoke" crates runtime sysroot tests/fixtures --glob "*.rs" --glob "*.c" --glob "*.h" --glob "*.scoop"` 确认 stackmap smoke 的实际测试入口在 `crates/scoopc/src/llvm/tests/abi.rs`，runtime 实现位于 `runtime/c/scoop_test.c`。
- 与 `PLAN.md` 对应闭合：
