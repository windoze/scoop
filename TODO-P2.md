...
- 依赖：P2-T01R
- 完成记录：
  - 2026-05-02：完成 `P2-T02`，将 `sysroot/core.scoop` 中的 `Continuation` surface 对齐到 `interface Continuation<Resume, Answer, eff E = Pure>`，并重写注释，明确源码层只保留普通 `k.resume(...)` 调用、`resume()` 只是 `resume(())` 的 sugar、tuple payload 必须作为单一 tuple 实参传递。
  - `crates/scoopc/src/typecheck/expr/call.rs` 现在把一般性的单一 `Unit` 参数 zero-arg sugar 收口到 typed call 解析：顶层 direct-call、extension/member-call、函数值调用、funptr 调用、effect-op 调用与 `Continuation.resume(...)` 都复用同一条“只有选中的 callable 恰好接收一个 `Unit` 参数时才插入显式 `UnitLit`”规则，而不是按 `resume` 名字开特例。
  - 针对重载解析补齐了 exact-arity 优先级：当 `f()` 已经有真正的零参候选时，typed 阶段不会误回退到 `f(())`；只有普通零参匹配失败时，才会把单一 `Unit` 参数候选当作 sugar fallback。该规则同样覆盖 extension/member-call 路径。
  - `crates/scoopc/src/ast/mod.rs`、`typecheck/lower.rs`、`typecheck/expr/entry.rs`、`hir/lower/expr.rs` 与 `hir/lower/mod.rs` 已通过 side table + typed HIR canonicalization 把 `k.resume()`、`f()`（命中单一 `Unit` 参数时）以及显式 `f(())` 统一落成显式 `UnitLit` 参数调用形状；AST / parser 仍保持原始零参 surface，不做提前改写。
  - 新增/更新测试与样本：`tests/fixtures/typecheck/continuation_resume_unit_sugar_ok.scoop`、`unit_single_param_zero_arg_call_ok.scoop`、`unit_single_param_zero_arg_extension_call_ok.scoop`、`unit_single_param_zero_arg_overload_prefers_exact_zero_arg_ok.scoop`、`continuation_resume_tuple_requires_single_tuple_arg_is_error.scoop`、`continuation_resume_answer_expression_ok.scoop`、`continuation_resume_surface_ok.scoop`、`tests/fixtures/hir/continuation_resume_surface_named_tuple_and_unit_basic.{scoop,hir}`、`tests/fixtures/run-pass/continuation_resume_surface_named_tuple_and_unit_basic.scoop`，并在 `crates/scoopc/src/typecheck/expr/entry.rs` / `crates/scoopc/src/hir/lower/mod.rs` 新增定向单元测试覆盖 overload 优先级、extension sugar 与 typed HIR canonical 形状。
...
- 依赖：P2-T02
- 完成记录：
  - 2026-05-02：完成 `P2-T02R` review，未发现需要在 `P2-T03` 前补入的新前置缺陷；最近一次提交 `[P2-T02] Align continuation surface and typed Unit sugar` 未显式留下与本 review 直接相关的未完成事项。
  - surface / sysroot 复核结论：`sysroot/core.scoop` 已把 `Continuation` 固定为 `interface Continuation<Resume, Answer, eff E = Pure>`，注释明确说明源码层只保留普通 `k.resume(...)` 调用、`k.resume()` 只是 `k.resume(())` 的 sugar、更一般的单一 `Unit` 参数 callable 允许 `f()` 作为 `f(())` 的 sugar，且 tuple payload 必须作为单一 tuple 实参传递；`resume(...): Answer / (E + Raise<RuntimeError>)` 也继续以 ordinary effect contract 暴露在 surface 上。
  - typed sugar / HIR 复核结论：`crates/scoopc/src/typecheck/expr/call.rs` 中 `can_use_zero_arg_unit_call_sugar(...)` 已被 direct-call、member/extension-call、函数值调用与 `Continuation.resume(...)` 统一复用；direct/member overload 解析都会先尝试 exact-arity 映射，只有在普通零参匹配失败且候选恰好接收单一 `Unit` 参数时才 fallback 到 sugar。`try_infer_continuation_resume_call_expr_type(...)` 仅接受单个 `value` 参数，并通过同一 helper 处理 `k.resume()`，未再保留 tuple payload 扁平展开或多参数 resume 特例。`crates/scoopc/src/hir/lower/mod.rs` 的 typed HIR 测试与 `dump-hir` 复验也确认 `k.resume()` / `k.resume(())`、`takesUnit()` / `takesUnit(())` 都已 canonicalize 为显式 `Unit` 实参调用形状。
  - AST / parser 复核结论：`crates/scoopc/src/parser/tests.rs` 仍直接断言 `k.resume(x)` 是普通 `Call + MemberAccess`、`k.resume()` 在 AST 中保持零参数调用；`crates/scoopc/src/ast/mod.rs` 与 `crates/scoopc/src/parser/file.rs` 中新增的 `zero_arg_unit_call_sugar_sites` 仅是 typecheck 回写的 side-table 挂点与初始化，注释也明确 sugar 发生在 typed 阶段，未把 zero-arg rewrite 下沉到 parser/AST 业务逻辑。
...
- 依赖：P2-T02R
- 完成记录：
  - 2026-05-02：完成 `P2-T03`，把 `Continuation<ResumeTuple, Answer, eff Out>` 的 typed receiver contract、runtime error ordinary effect 传播，以及 compiler-owned interface 约束统一收口到 refactor typed HIR / typecheck 阶段。
  - `crates/scoopc/src/typecheck/expr/call.rs` 继续以 `Continuation.resume(...)` 的显式 receiver contract 作为 authoritative typed 语义来源：`resume` 的 payload 类型来自 `ResumeTuple`，返回类型来自 `Answer`，receiver 上的 `Out` effect row 继续作为 resumed computation 的 outward effects；同时对调用点显式记录 `Raise<RuntimeError>` ordinary effect，而不是隐藏错误通道。
  - `crates/scoopc/src/effect_refactor_pipeline/hir_stage.rs` 不再只暴露 placeholder `TypedHirEffectContracts`；现已新增 `ContinuationResumeSiteContract` 与显式 `continuation_resume_sites` side table，把每个 typed `Continuation.resume(...)` 站点的 `receiver_ty`、`ResumeTuple`、`Answer`、`return_ty`、`Out` effect row 以及 “required effects 包含 `Raise<RuntimeError>`” 这一事实稳定写入 refactor typed HIR stage 输出，供 P3 直接消费而无需回 AST/typecheck 猜测。
  - `crates/scoopc/src/typecheck/interfaces.rs` 新增 `continuation_impl_not_allowed` 门禁；class/object/struct/enum/interface 一旦试图实现或继承 `scoop.core.Continuation`，会在 interface/typecheck 阶段直接被拒绝。`crates/scoopc/src/typecheck/expr/error.rs` 与 `typecheck/expr/call.rs` 新增 `continuation_not_constructible` 门禁，显式拒绝 `Continuation<...>()` 这类用户态运行期构造。
  - 新增/更新测试与 fixtures：`crates/scoopc/src/effect_refactor_pipeline/hir_stage.rs`、`crates/scoopc/src/typecheck/interfaces.rs`、`crates/scoopc/src/typecheck/expr/entry.rs`、`crates/scoop/src/commands/dump_hir.rs`，以及 `tests/fixtures/typecheck/continuation_user_impl_is_error.scoop`、`continuation_runtime_ctor_is_error.scoop`、`continuation_resume_requires_runtime_error_effect_is_error.scoop`。
  - 本任务未改动 `TODO.md` 或 `PLAN.md`：任务索引、顺序与阶段计划保持不变。
...
- 依赖：P2-T03
- 完成记录：
  - 2026-05-02：完成 `P2-T03R` review，未发现需要在 `P2-T04` 前补入的新前置缺陷；最近一次提交 `[P2-T03] Surface continuation typed contracts` 也未显式留下与本 review 直接相关的未完成事项。
  - refactor typed contract 复核结论：`sysroot/core.scoop` 已把 `Continuation<Resume, Answer, eff E = Pure>` 固定为 compiler-owned interface，并显式声明 `resume(value): Answer / (E + Raise<RuntimeError>)`；`crates/scoopc/src/typecheck/expr/call.rs` 中 `try_infer_continuation_resume_call_expr_type(...)` 继续以 receiver 上的 `ResumeTuple` / `Answer` / `Out` 作为 authoritative typed contract 来源，对 `k.resume(...)` 的 payload 类型、返回类型与 safe-call `Option<Answer>` 包装做显式检查；`crates/scoopc/src/effect_refactor_pipeline/hir_stage.rs` 的 `ContinuationResumeSiteContract` / `continuation_resume_sites` side table 已把 `receiver_ty`、`ResumeTuple`、`Answer`、`return_ty`、`Out` effect row 与“required effects 包含 `Raise<RuntimeError>`”写入 refactor typed HIR stage 输出。
  - runtime error ordinary effect 复核结论：refactor 路径在 `try_infer_continuation_resume_call_expr_type(...)` 中显式把 receiver 的 `Out` effect row 逐项 `record_performed_effect(...)`，随后额外 lowering `scoop.core.RuntimeError` / `scoop.core.Raise` 并记录 `Raise<RuntimeError>`，说明 runtime error 仍通过 ordinary required-effect 通道传播，而不是隐藏错误通道；typed HIR stage 侧通过 `ContinuationResumeSiteContract::required_effects_include_runtime_error()` 把该事实稳定暴露给下游。
  - compiler-owned 约束复核结论：`crates/scoopc/src/typecheck/interfaces.rs` 的 `reject_compiler_owned_continuation_interface(...)` 会在 class/object/struct/enum/interface 的 supertype 检查阶段直接拒绝实现或继承 `scoop.core.Continuation`；`crates/scoopc/src/typecheck/expr/call.rs` 在 nominal ctor 解析失败后会通过 `resolves_to_compiler_owned_continuation_type(...)` + `ExprTypeError::ContinuationNotConstructible` 拒绝 `Continuation<...>()` 这类用户态构造；相关定向测试继续通过。
...
- 依赖：P2-T03R
- 完成记录：
  - 2026-05-02：完成 `P2-T04`，把 refactor typed HIR stage 的 side tables 从“仅有 `Continuation.resume(...)` 占位”扩展为完整的 `TypedHirEffectContracts`：现已显式输出 `function_effects`（函数级 allowed-row / required-effects contract）、`call_site_kinds`、`continuation_resume_sites`、`perform_sites`、`handle_sites`，并且 `Continuation.resume(...)` 站点会稳定暴露 `ResumeTuple` / `Answer` / `Out` 与 `Raise<RuntimeError>` ordinary effect 贡献，`perform` 站点会暴露 concrete op / performed effect / payload contract，`handle` 站点会暴露 handle body result type 与 arm effect/result typed 关系。
  - `crates/scoopc/src/effect_refactor_pipeline/hir_stage.rs` 现已提供稳定 formatter：`TypedHirEffectContracts::stable_dump(...)` 与 `TypedHirStageOutput::stable_dump()` 使用 `TypeStore` 把 side tables 渲染为稳定文本，并按确定顺序输出函数合同、调用点分类、continuation contract、perform contract、handle contract，不再依赖 `HashMap` 迭代顺序。
  - `crates/scoop/src/commands/dump_hir.rs` 已更新为：`legacy` 路径继续只打印 legacy `HIR File` Debug；`refactor` 路径则在打印 typed HIR `File` Debug 后追加上述 typed contract 区块，从而让 `dump-hir --effect-pipeline refactor` 直接成为 P2 -> P3 handoff 的可视化验证入口，而不是只看到主体 HIR 结构。
  - 新增/更新测试与样本：`crates/scoopc/src/effect_refactor_pipeline/hir_stage.rs` 现已包含 `refactor_typed_hir_*` continuation/runtime-error/handle snapshot 与结构测试；`crates/scoop/src/commands/dump_hir.rs` 新增 refactor dump 输出断言；新增 `tests/fixtures/hir/continuation_runtime_error_surface_basic.{scoop,hir}`，并继续复用 `continuation_resume_surface_named_tuple_and_unit_basic.scoop` 与 `handle_perform.scoop` 锁定 dump 行为。
  - 本任务未改动 `TODO.md` 或 `PLAN.md`：任务顺序、索引与阶段计划保持不变。
...
- 依赖：P2-T04
- 完成记录：
  - 2026-05-02：完成 `P2-T04R` review，未发现需要在进入 `P3` 前补入的新前置缺陷；最近一次提交 `[P2-T04] Emit typed HIR effect contract tables` 也未显式留下与本 review 直接相关的未完成事项。
  - stage / handoff 复核结论：`crates/scoopc/src/effect_refactor_pipeline/hir_stage.rs` 已把 `TypedHirStageOutput` 与 `TypedHirEffectContracts` 固化为 P2 -> P3 的显式 handoff，稳定暴露 `function_effects`、`call_site_kinds`、`continuation_resume_sites`、`perform_sites`、`handle_sites`，并通过 `TypedHirStageOutput::stable_dump()` / `TypedHirEffectContracts::stable_dump(...)` 以确定顺序输出 typed contract；这些 side tables 已足以承载 `resume` 的 `ResumeTuple` / `Answer` / `Out` / runtime-error effect、`perform` 的 op/payload contract、以及 `handle` 的 result/arm/finally 关系，满足 `TODO-P3.md` 对“P3 不再回 AST/typecheck 猜语义”的前置要求。
  - 路由复核结论：`crates/scoop/src/commands/dump_hir.rs` 仍在命令边界显式分流为 `legacy => scoopc::hir::lower_for_dump(...)`、`refactor => scoopc::effect_refactor_pipeline::load_typed_hir_stage_output_for_dump(...)`；`crates/scoopc/src/effect_refactor_pipeline/mod.rs` 继续把 refactor `dump-hir` 收口到 typed HIR stage，而不是换壳调用 legacy `lower_for_dump`，因此 `dump-hir --effect-pipeline refactor` 已可直接作为 P2 handoff 的可视化验证入口。
  - continuation / runtime-error / compiler-owned 约束复核结论：`sysroot/core.scoop` 继续以 `interface Continuation<Resume, Answer, eff E = Pure>` 暴露 surface，并把 `resume(value): Answer / (E + Raise<RuntimeError>)` 固定为 ordinary effect contract；`crates/scoopc/src/typecheck/expr/call.rs` 的 `try_infer_continuation_resume_call_expr_type(...)` 明确记录 receiver `Out` effect row 与额外 `Raise<RuntimeError>` required effect，并把 `k.resume()` / 单一 `Unit` 参数 zero-arg sugar 收口到 typed 阶段；`crates/scoopc/src/typecheck/interfaces.rs` 与构造调用路径继续在 typecheck 阶段拒绝用户实现/构造 compiler-owned `Continuation`，说明 P3 不需要再回到 AST/parser 或 codegen 补 continuation 语义。
  - 搜索摘要：执行 `rg "EffectPipelineMode|effect_pipeline|legacy|refactor" crates/scoopc/src/hir crates/scoopc/src/typecheck` 后，`hir/` 命中仅来自既有 `legacy_eager_hir` 命名与注释，`typecheck/` 命中仅来自 refactor 定向测试与 legacy continuation 简写迁移诊断文本；未发现 pipeline selector 下沉到旧 HIR/typecheck 业务函数中的新增分支。
  - 复验通过：`cargo test -p scoopc --no-default-features refactor_typed_hir_stage`、`cargo test -p scoopc --no-default-features effect_refactor_pipeline`、`cargo test -p scoop --no-default-features dump_hir`、`cargo test -p scoop --no-default-features parity`、`cargo test -p scoopc --no-default-features continuation_resume`、`cargo test -p scoopc --no-default-features unit_single_param_zero_arg`、`cargo test -p scoopc --no-default-features refactor_continuation_typecheck`、`cargo test -p scoopc --no-default-features refactor_typed_hir`、`cargo run -q -p scoop --no-default-features -- --effect-pipeline legacy dump-hir tests/fixtures/hir/minimal.scoop`、`cargo run -q -p scoop --no-default-features -- --effect-pipeline refactor dump-hir tests/fixtures/hir/minimal.scoop`、`cargo run -q -p scoop --no-default-features -- --effect-pipeline refactor dump-hir tests/fixtures/run-pass/continuation_resume_surface_named_tuple_and_unit_basic.scoop`、`cargo run -q -p scoop --no-default-features -- --effect-pipeline refactor test --fixtures tests/fixtures/typecheck/continuation_resume_answer_expression_ok.scoop`、`cargo run -q -p scoop --no-default-features -- --effect-pipeline refactor test --fixtures tests/fixtures/typecheck/continuation_resume_unit_sugar_ok.scoop`、`cargo run -q -p scoop --no-default-features -- --effect-pipeline refactor test --fixtures tests/fixtures/typecheck/unit_single_param_zero_arg_call_ok.scoop`、`cargo run -q -p scoop --no-default-features -- --effect-pipeline refactor test --fixtures tests/fixtures/typecheck/unit_single_param_zero_arg_extension_call_ok.scoop`、`cargo run -q -p scoop --no-default-features -- --effect-pipeline refactor test --fixtures tests/fixtures/typecheck/unit_single_param_zero_arg_overload_prefers_exact_zero_arg_ok.scoop`、`cargo run -q -p scoop --no-default-features -- --effect-pipeline refactor test --fixtures tests/fixtures/typecheck/continuation_resume_tuple_requires_single_tuple_arg_is_error.scoop`、`cargo run -q -p scoop --no-default-features -- --effect-pipeline refactor test --fixtures tests/fixtures/typecheck/continuation_answer_type_mismatch_is_error.scoop`、`cargo run -q -p scoop --no-default-features -- --effect-pipeline refactor test --fixtures tests/fixtures/typecheck/continuation_resume_in_pure_main_after_handle_is_error.scoop`、`cargo run -q -p scoop --no-default-features -- --effect-pipeline refactor test --fixtures tests/fixtures/typecheck/continuation_resume_from_escape_binder_requires_step_effect.scoop`、`cargo run -q -p scoop --no-default-features -- --effect-pipeline refactor test --fixtures tests/fixtures/typecheck/continuation_resume_required_effect_missing_is_error.scoop`、`cargo run -q -p scoop --no-default-features -- --effect-pipeline refactor test --fixtures tests/fixtures/typecheck/continuation_resume_requires_raise_runtime_error_missing_is_error.scoop`、`cargo run -q -p scoop --no-default-features -- --effect-pipeline refactor test --fixtures tests/fixtures/typecheck/continuation_user_impl_is_error.scoop`、`cargo run -q -p scoop --no-default-features -- --effect-pipeline refactor test --fixtures tests/fixtures/typecheck/continuation_runtime_ctor_is_error.scoop`、`cargo run -q -p scoop --no-default-features -- --effect-pipeline refactor test --fixtures tests/fixtures/typecheck/continuation_resume_requires_runtime_error_effect_is_error.scoop`、`cargo run -q -p scoop --no-default-features -- --effect-pipeline refactor test --fixtures tests/fixtures/hir/continuation_resume_surface_named_tuple_and_unit_basic.scoop`、`cargo run -q -p scoop --no-default-features -- --effect-pipeline refactor dump-hir tests/fixtures/hir/continuation_resume_surface_named_tuple_and_unit_basic.scoop`、`cargo run -q -p scoop --no-default-features -- --effect-pipeline refactor dump-hir tests/fixtures/hir/continuation_runtime_error_surface_basic.scoop`、`cargo run -q -p scoop --no-default-features -- --effect-pipeline refactor dump-hir tests/fixtures/hir/handle_perform.scoop`、`cargo run -q -p scoop --no-default-features -- --effect-pipeline legacy dump-hir tests/fixtures/hir/handle_perform.scoop`、`cargo run -q -p scoop --no-default-features -- test --fixtures tests/fixtures/hir/continuation_runtime_error_surface_basic.scoop`、`cargo clippy -p scoop -p scoopc --all-targets --no-default-features -- -D warnings`。
