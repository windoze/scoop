...
## 2. `Task` core 已定型为通用的 pollable object；当前剩余缺口只剩 stable handle 作为 wake token / reactor identity 的合同

- 现状：`Task<T>` 现已作为普通、lazy、可手动 `poll()/step()` 的 `scoop.core.Task` 对外成立；`async {}` / `async fun` 统一 lower 到内部 task-create + step-result 主线，公开的 `scoop.task` executor surface、runtime executor implementation 与 compiler 端 `scoop.task.*` / `Executor` special-case 都已移除。`T4009c` 进一步把公开 `spawn/join` surface 明确降回 deferred structured-concurrency 议题：parser / AST 仍保留语法壳以便后续恢复，但 typecheck 会统一报 `structured_concurrency_deferred`，文档也不再把它们当成当前 `Task` core 的一部分。与此同时，验证时额外发现并修复了一个既有 lowering 裂缝：当 `async` 体尾值直接是 `await task` 且结果为 GC 引用类型时，结果会在进入 `step_ready(...)` 前丢失；当前已通过“先物化 async body 尾值，再进入 `__task_ready_value` / `step_ready(...)`”的方式与既有 `Task<Int>` 主线对齐。
- 影响：`Task` 现在已经真正脱离 executor 与 structured-concurrency 前提，可以单独作为通用 pollable object 成立；下一项剩余 issue 不再是 `Task` 对象模型本身，而是 reactor / OS callback / future executor 需要的 stable handle wake-token 合同，以及它与 `Pinned` 的职责划分。
- 证据：`sysroot/core.scoop`；`SCOOP_FULL_SPEC.md` 第 5.7 节；`SCOOP_RUNTIME.md` 第 11 节；`crates/scoopc/src/typecheck/expr/error.rs`；`crates/scoopc/src/typecheck/expr/infer.rs`；`crates/scoopc/src/hir/lower/block.rs`；`runtime/c/scoop_task.c`；`tests/fixtures/typecheck/spawn_deferred_is_error.scoop`；`tests/fixtures/typecheck/join_deferred_is_error.scoop`；`tests/fixtures/run-pass/async_await_string_basic.scoop`；`tests/fixtures/run-pass/task_poll_step_manual_basic.scoop`。
