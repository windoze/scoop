...
- 影响：编译期执行链路的“调用绑定 + generic 实例化 + 常见纯控制流 + const 声明级 effect contract”主线已基本打通；后续重点收窄到 `when`、lambda、effectful constructs 等更复杂纯语义，而不是继续反复定义 `const fun` 头部的 effect-row 规则。
