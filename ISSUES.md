...
## 2. `Task` 仍未定型为通用的 pollable object；剩余缺口已收窄到 `poll()` 合同

- 现状：`Task<T>` 现已作为 `scoop.core.Task` 对外成立；`async {}` / `async fun` 统一 lower 到内部 lazy create helper，公开的 `scoop.task` executor surface 与 `runtime/c/scoop_task_executor.c` 已移除。`T4009a3` 之后，LLVM/codegen 中只服务旧 `scoop.task.*` / `Executor` 方向的 hard-coded special-case 与 runtime symbol declarations 也已清理完毕，当前只保留内部 `__scoop_task_create` / `__scoop_task_from_result` / `__scoop_task_join` helper。剩余未定型的部分已经收窄为：公开的 `Task.poll()` / `step()` / `Poll<T>` 合同仍未定义，当前只能依赖过渡期内部 helper（例如 `join`）驱动 lazy task。
- 影响：`Task` 已经不再被旧 handle ABI、公开 executor surface 或 compiler 侧陈旧 special-case 绑住；但在 `poll()` 合同真正落地前，它仍不是完整定型的通用 pollable object，也还没有把 suspended-state carrier 的公开/私有边界彻底固定下来。
- 证据：`sysroot/core.scoop:454-479`；`crates/scoopc/src/typecheck/expr/infer.rs:478-603`；`crates/scoopc/src/hir/lower/mod.rs:126-129`；`crates/scoopc/src/hir/lower/expr.rs:492-571`；`crates/scoopc/src/llvm/codegen/mod.rs:2431-2435`；`crates/scoopc/src/llvm/codegen/runtime_symbols.rs:104-106`；`crates/scoopc/src/llvm/codegen/runtime_abi.rs:733-775`；`runtime/c/scoop_task.c:1-247`；`tests/fixtures/run-pass/async_fun_task_runtime_basic.scoop:1-30`。
