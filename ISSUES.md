...
## 2. `Task` 仍未定型为通用的 pollable object；剩余缺口主要在 `poll()` 合同与遗留 special-case 清理

- 现状：`Task<T>` 现已作为 `scoop.core.Task` 对外成立；`async {}` / `async fun` 统一 lower 到内部 lazy create helper，公开的 `scoop.task` executor surface 与 `runtime/c/scoop_task_executor.c` 也已移除。剩余未定型的部分主要有两类：其一，公开的 `Task.poll()` / `step()` / `Poll<T>` 合同仍未定义，当前只能依赖过渡期内部 helper（例如 `join`）驱动 lazy task；其二，LLVM/codegen 里仍保留一组只服务旧 `scoop.task.*` / `Executor` 方向的 hard-coded special-case 与 runtime symbol declarations，尚未在 `T4009a3` 中清理。
- 影响：`Task` 已经不再被旧 handle ABI 或公开 executor surface 绑住，但在 `poll()` 合同真正落地前，它仍不是完整定型的通用 pollable object；同时，遗留 special-case 继续扩大实现表面积，阻碍“Task 只是普通对象 + 私有 suspended-state carrier”这一叙事彻底收口。
- 证据：`sysroot/core.scoop:454-479`；`crates/scoopc/src/typecheck/expr/infer.rs:478-603`；`crates/scoopc/src/hir/lower/mod.rs:126-129`；`crates/scoopc/src/hir/lower/expr.rs:492-571`；`crates/scoopc/src/llvm/codegen/mod.rs:2432-2469`；`crates/scoopc/src/llvm/codegen/runtime_symbols.rs:53-57`；`crates/scoopc/src/llvm/codegen/runtime_abi.rs:735-905`；`runtime/c/scoop_task.c:1-247`；`tests/fixtures/run-pass/async_fun_task_runtime_basic.scoop:1-30`。
