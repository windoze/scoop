// Scoop C runtime: std `scoop.channels` (pthread backend, early stage).
//
// TODO T1319d：
...
void *scoop_channels_channel_create(void) {
  ...
  (void)pthread_mutex_init(&ch->lock, 0);
  (void)pthread_cond_init(&ch->cond, 0);
  ch->closed = 0;
  ch->_reserved_u32 = 0;
  ch->head = 0;
  ch->tail = 0;
...
uint32_t scoop_channels_send_u64(void *channel_obj, uint64_t value) {
  ...
  (void)pthread_mutex_lock(&ch->lock);
  ...
  (void)pthread_cond_signal(&ch->cond);
  (void)pthread_mutex_unlock(&ch->lock);
  return 1;
}
...
uint32_t scoop_channels_recv_u64(void *channel_obj, uint64_t *out_value) {
  ...
  while (ch->head == 0 && !ch->closed) {
    (void)pthread_cond_wait(&ch->cond, &ch->lock);
  }
...
void scoop_channels_close(void *channel_obj) {
  ...
  (void)pthread_mutex_lock(&ch->lock);
  ch->closed = 1;
  (void)pthread_cond_broadcast(&ch->cond);
  (void)pthread_mutex_unlock(&ch->lock);
}
