...
// --- Task<T> / executor（最小原语，spec §5.7 / TODO T0917） ---
//
// 目标：
// - 提供支撑 `Task<T>`/`async`/`spawn` 的最小 runtime 原语：
//   - task 状态机（created/scheduled/running/completed）
//   - executor 队列：入队 continuation + resume（u64 payload）
//   - task completion 回调：完成后把等待者的 continuation 入队到指定 executor
//   - 可选显式 start：将 task 的 body 入队到 executor 运行
//
// 设计说明（early stage）：
// - `Task<T>` 在当前编译器阶段仍落到 “word-sized handle”（`uint64_t`），这里用指针句柄实现。
// - 为了避免 GC 在 task/queue 持有 continuation 指针期间把 continuation sweep 掉，这里使用
//   `scoop_pin/scoop_unpin` 把 continuation 当作额外 roots：
//   - enqueue 或注册回调时 pin
//   - job 执行并完成 resume 后 unpin
