...
impl<'a, 'ctx> MainCodegen<'a, 'ctx> {
        ...
        match fqn {
            "scoop.unsafe.__atomicIntLoad" => {
                let target_expr = self.expect_hir_positional_intrinsic_arg(
                    ...
                    1,
                    0,
                    "codegen_sysroot_atomic_int_load",
                );
                ...
                let raw = self.expect_int_value(loaded, "codegen_sysroot_atomic_int_load return");
            ...
            "scoop.unsafe.__atomicIntStore" => {
                let target_expr = self.expect_hir_positional_intrinsic_arg(
                    ...
                    2,
                    0,
                    "codegen_sysroot_atomic_int_store",
                );
                let value_expr = self.expect_hir_positional_intrinsic_arg(
                    args,
                    2,
                    1,
                    "codegen_sysroot_atomic_int_store",
                ...
                let (raw_int, from) =
                    self.expect_cg_int(v, "codegen_sysroot_atomic_int_store value");
            ...
            "scoop.unsafe.__atomicIntCompareExchange" => {
                let target_expr = self.expect_hir_positional_intrinsic_arg(
                    ...
                    3,
                    0,
                    "codegen_sysroot_atomic_int_cmpxchg",
                );
                let expected_expr = self.expect_hir_positional_intrinsic_arg(
                    args,
                    3,
                    1,
                    "codegen_sysroot_atomic_int_cmpxchg",
                );
                let desired_expr = self.expect_hir_positional_intrinsic_arg(
                    args,
                    3,
                    2,
                    "codegen_sysroot_atomic_int_cmpxchg",
                ...
                let (expected_raw, expected_from) =
                    self.expect_cg_int(expected_v, "codegen_sysroot_atomic_int_cmpxchg expected");
                ...
                let (desired_raw, desired_from) =
                    self.expect_cg_int(desired_v, "codegen_sysroot_atomic_int_cmpxchg desired");
                ...
                let ok =
                    self.expect_int_value(success, "codegen_sysroot_atomic_int_cmpxchg success");
                Ok(CgValue::bool(ok))
            }
            _ => self.panic_verified_intrinsic_contract(
                "codegen_sysroot_atomic_int_intrinsics",
                "unknown atomicInt callee",
            ),
        }
    }
        ...
        match atomic_intrinsic_base_fqn(fqn) {
            "scoop.unsafe.__atomicRefLoad" => {
                let target_expr = self.expect_hir_positional_intrinsic_arg(
                    ...
                    1,
                    0,
                    "codegen_sysroot_atomic_ref_load",
                );
            ...
            "scoop.unsafe.__atomicRefStore" => {
                let target_expr = self.expect_hir_positional_intrinsic_arg(
                    ...
                    2,
                    0,
                    "codegen_sysroot_atomic_ref_store",
                );
                let value_expr = self.expect_hir_positional_intrinsic_arg(
                    args,
                    2,
            ...
            "scoop.unsafe.__atomicRefCompareExchange" => {
                let target_expr = self.expect_hir_positional_intrinsic_arg(
                    ...
                    3,
                    0,
                    "codegen_sysroot_atomic_ref_cmpxchg",
                );
                let expected_expr = self.expect_hir_positional_intrinsic_arg(
                    args,
                    3,
                ...
                let success = self.builder.build_extract_value(cx, 1, "cmpxchg_success")?;
                let ok =
                    self.expect_int_value(success, "codegen_sysroot_atomic_ref_cmpxchg success");
                self.codegen_atomic_ref_cas_barrier(desired_expr.span, ok, desired)?;
                Ok(CgValue::bool(ok))
            }
            _ => self.panic_verified_intrinsic_contract(
                "codegen_sysroot_atomic_ref_intrinsics",
                "unknown atomicRef callee",
            ),
        }
    }
    ...
    fn codegen_atomic_ref_lvalue_place(
        ...
        if mode == AtomicRefLvalueMode::ReadWrite && !place.writable {
            self.panic_verified_intrinsic_contract(
                "codegen_atomic_ref_lvalue_place",
                "target is not writable",
            );
        }
