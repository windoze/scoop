...
pub(in crate::typecheck::expr) fn pick_most_specific_param_types(
    candidates: &[&[TypeId]],
    lower: &TypeLowering<'_>,
    builtins: BuiltinTypes,
) -> Option<usize> {
    ...
    'candidate: for idx in 0..candidates.len() {
        for other_idx in 0..candidates.len() {
            ...
            if param_types_strictly_more_specific(
...
pub(in crate::typecheck::expr) fn pick_most_exact_param_match(
    candidates: &[&[TypeId]],
    target: &[TypeId],
) -> Option<usize> {
    ...
    for (idx, params) in candidates.iter().enumerate() {
        ...
        let score = params
            ...
            .filter(|(param, target)| param == target)
            .count();
        match best {
            None => {
                best = Some((idx, score));
    ...
    match (best, tied) {
        (Some((idx, score)), false) if score > 0 => Some(idx),
        _ => None,
    }
}

fn param_types_strictly_more_specific(
    lhs: &[TypeId],
    rhs: &[TypeId],
    lower: &TypeLowering<'_>,
    builtins: BuiltinTypes,
    ...
    for (lhs_ty, rhs_ty) in lhs.iter().copied().zip(rhs.iter().copied()) {
        if !is_type_assignable(lhs_ty, rhs_ty, lower, builtins) {
            return false;
        }
        if !is_type_assignable(rhs_ty, lhs_ty, lower, builtins) {
            strict = true;
        }
    }
    strict
