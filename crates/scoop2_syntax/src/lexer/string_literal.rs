//! 字符串字面量解析（early stage，自 `scoopc_ast::syntax::string_literal` 移植）。
//!
//! 说明：
...
///
/// 支持：
/// - bare `{` / `}` are literal f-string text;
...
pub fn parse_f_string_text_bytes(
    ...
    // 非 raw：复用普通字符串的转义解析；`{` / `}` 在 f-string 文本中为普通字符。
    parse_normal_string_bytes(text)
}
