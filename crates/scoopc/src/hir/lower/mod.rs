...
pub fn lower_typed_for_dump(
    ...
    let index = {
        ...
        for file in &session.sysroot().compilable_files {
            compilation_unit.push((&file.source, &file.ast));
        }
    ...
    // T0143：dump-mode 需把 overlay `core.scoop` 等 compilable sysroot 文件
    // 纳入 TypeEnv，否则 `lower_type_ref_in_decl_file` 找不到这些声明对应的源
    // 文件会回落到调用方 source 切片越界。
    for f in &session.sysroot().compilable_files {
