...
impl<'a> HirLowering<'a> {
            ...
            ast::ExprKind::Call { callee, args } => {
                    ...
                    self.try_lower_effect_op_call_expr(pkg_prefix, e.span, callee, args)
                {
                    ...
                    let receiver = if let ast::ExprKind::Ident(id) = &receiver.kind
                        && id.resolved.is_none()
                        && !matches!(self.source.slice(id.span), "this" | "true" | "false")
                    {
                        if !owner_is_object {
                            return None;
                        }
    ...
    fn typechecked_direct_call_expr(
        &mut self,
        span: Span,
        binding: &crate::ast::TopLevelFunCallBinding,
        args: Vec<CallArg>,
    ...
    fn try_lower_typechecked_compare_to_binary_expr(
        &mut self,
        pkg_prefix: &str,
        span: Span,
        op: ast::BinaryOp,
    ...
    fn should_keep_member_call_as_member_access(
        ...
        // P4-T01l：scalar `toString` 已经迁到 `@Intrinsic struct/class` body method
        // override，HIR 不再保留 MemberAccess 形态——直接走 T1508a 主线把
        // `<scalar>.toString()` 改写成顶层 `<TypeFqn>.toString(receiver)` 调用，
        // 让 sysroot bodied helper 接管。剩下的 hash / toInt / abs / ... 仍依赖
        // by-name 拦截，留给 P4-T01 / 后续任务收口。
