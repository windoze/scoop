...
//! - 支持 `const val` initializer 的常量折叠（用于旧 const-eval 回归）。
//!
//! 非目标（后续任务逐步补齐）：
...
impl<'a> ConstInterpreter<'a> {
    ...
    fn call_fun_or_intrinsic(
        ...
        let selected_binding = self.top_level_fun_call_binding_for_current_file(call_span);
    ...
    fn eval_for_like_loop(
        ...
        // const-eval `for`：先求值 iter，再逐次把 binder 绑定到当前元素。
