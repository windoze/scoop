...
fn trim_unit_files(files: &[(&str, &str)]) -> (Sysroot, Vec<SourceFile>, Vec<crate::ast::File>) {
    let sysroot = load_sysroot();
    let sources = files
        .iter()
        .map(|(path, text)| SourceFile::new_virtual(*path, (*text).to_string()))
        .collect::<Vec<_>>();
    let mut asts = sources
        .iter()
        .map(|source| parser::parse_file(source).expect("parse"))
        .collect::<Vec<_>>();
    {
        ...
        .expect("trim package-level comptime ifs");
    }
    (sysroot, sources, asts)
}

fn assert_trimmed_fun_returns_int(
    source: &SourceFile,
    file: &crate::ast::File,
    fun_name: &str,
    expected_return_expr_text: &str,
) {
    assert!(
        !file
            .items
            .iter()
            .any(|item| matches!(item, crate::ast::Item::ComptimeIf(_))),
        "trim 后不应残留 package-level comptime if"
    );
    ...
    let fun = file
        ...
        .find_map(|item| match item {
            crate::ast::Item::Fun(fun) if source.slice(fun.name.span) == fun_name => Some(fun),
            _ => None,
        })
        .unwrap_or_else(|| panic!("trim 后缺少函数 `{fun_name}`"));

    let crate::ast::FunBody::Block(body) = &fun.body else {
        panic!("trim 后函数 `{fun_name}` 缺少函数体");
    };
    let first_stmt = body.stmts.first().expect("trimmed fun body stmt");
    let crate::ast::StmtKind::Return {
        value: Some(return_expr),
        ..
    } = &first_stmt.kind
    else {
        panic!("trim 后函数 `{fun_name}` 第一条语句应为 return");
    };

    assert_eq!(source.slice(return_expr.span), expected_return_expr_text);
}
...
#[test]
fn package_level_comptime_if_same_file_overloaded_const_fun_condition_uses_typechecked_binding() {
    let (_sysroot, sources, asts) = trim_unit_files(&[(
        "main.scoop",
        r#"
const fun describe(x: Int): Bool {
    return true
}

const fun describe(x: String): Bool {
    return false
}

comptime if (describe(1)) {
    const fun pick(): Int {
        return 1
    }
} else {
...
"#,
    ...
    assert_trimmed_fun_returns_int(&sources[0], &asts[0], "pick", "1");
}

#[test]
fn package_level_comptime_if_same_file_generic_const_fun_accepts_explicit_type_args() {
    let (_sysroot, sources, asts) = trim_unit_files(&[(
        "main.scoop",
        r#"
const fun <T> truthy(x: T): Bool {
    return true
}
...
"#,
    ...
    assert_trimmed_fun_returns_int(&sources[0], &asts[0], "pick", "7");
}

#[test]
fn package_level_comptime_if_cross_file_imported_const_fun_uses_compilation_unit_binding() {
    let (_sysroot, sources, asts) = trim_unit_files(&[
        (
            "helpers.scoop",
            r#"
...
public const fun enabled(x: Int): Bool {
    return true
}
"#,
        ),
...
"#,
    ...
    assert_trimmed_fun_returns_int(&sources[1], &asts[1], "pick", "42");
}
