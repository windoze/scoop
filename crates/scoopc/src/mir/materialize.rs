...
impl MirInstanceMaterializer {
    ...
    fn run(mut self, initial_requests: Vec<InstanceKey>) -> MaterializeResult<MaterializedMir> {
        ...
        // P4-T01l：materialized 实例（generic 单态化产物）的 body 内部仍可能包含对
        // 非泛型 callable 的 Direct 调用（典型例子：interface default body
        // `scoop.core.ToString.toString` 在 `print<T:ToString>` 单态化为
        // `print::<Int>` 后，只要 where-bound 没有被 statically devirtualize，
        // 该 callable 就仍会以 `Direct{ToString.toString}` 形态保留在 MIR 内）；
        // 而 `seed_request_root_direct_call_instances` 只会从 entry-point 起跑，
        // 不会自动跨 materialized 实例继续递归。这里在 run 末尾补一轮"对所有
        // materialized 实例 body 内的 Direct 调用做非泛型 callee scan"，确保
        // 非泛型 fun 体（含 interface default body）被正确发布到
        // `pass_published_ordinary_callables`。
        let materialized_snapshot: Vec<(InstanceKey, Vec<FunDecl>)> = self
            .materialized
            .iter()
            .map(|(k, v)| (k.clone(), v.clone()))
            .collect();
        for (instance, family) in materialized_snapshot {
            for fun in family {
                let Some(body) = fun.body.as_ref() else {
                    continue;
                };
                let scan_source = if instance.template.source_path.as_os_str().is_empty() {
                    PathBuf::new()
                } else {
                    instance.template.source_path.clone()
                };
                let mut to_scan: Vec<ReachableMirFun> = Vec::new();
                    ...
                    for stmt in &block.stmts {
                        ...
                        match value {
                            Rvalue::Call {
                                ...
                                if let Some(reachable_callee) = self
                                    .resolve_non_generic_direct_callee(
                                        scan_source.as_path(),
                                        stmt.span,
                                        callee_fqn,
                                        args,
                                ...
                                {
                                    to_scan.push(reachable_callee);
                                }
                            }
                            // P4-T01l：Interface / Virtual dispatch 在 itable / vtable 命中
                            // override impl 时，base interface method 自身（含 default body）
                            // 仍可能在下游 LLVM emit reachability（HIR 视图）被标记为 reachable，
                            // 因此把 `<owner_fqn>.<member_name>` 也作为非泛型 callee 发布候选；
                            // 这样 `interface ToString { fun toString(): String { return "" } }` 这类
                            // 默认 body 的 callable 会被正确收录到 `pass_published_ordinary_callables`，
                            // 与 P4-T01a/b/c 的 itable / vtable 收口不冲突（dispatch 仍按 override 命中）。
                            Rvalue::Call {
                                kind: CallKind::Interface { dispatch, .. },
                                ..
                            }
                            | Rvalue::Call {
                                ...
                                let interface_method_fqn = format!(
                                    "{}.{}",
                                    dispatch.owner_fqn, dispatch.member_name
                                );
                                if let Some(reachable_callee) = self
                                    .resolve_non_generic_fun_body_by_fqn(
                                        scan_source.as_path(),
                                        &interface_method_fqn,
                                    )
                                {
                                    to_scan.push(reachable_callee);
                                }
                ...
                for fun in to_scan {
                    self.scan_reachable_non_generic_fun(&fun, &mut sink)?;
                }
            }
        }
