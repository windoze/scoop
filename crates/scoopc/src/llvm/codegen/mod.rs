...
impl<'a, 'ctx> MainCodegen<'a, 'ctx> {
    ...
    fn declare_callee_resume_entry_function(
        ...
        resume_fun.set_gc(super::LLVM_GC_STRATEGY_STATEPOINT_EXAMPLE);
        if let Some(result_ty) = hidden_sret_result_ty {
    ...
    fn defer_gc_sensitive_cg_value(
        ...
        let mut gc_leaf_slots = Vec::new();
        self.collect_gc_ptr_leaf_slots_in_spill(slot, llvm_ty, name, &mut gc_leaf_slots)?;
        ...
        if !self.ordinary_effect_propagation_enabled() {
            ...
            let active_raw = self
                .build_call_preserving_gc_local_roots(
                    ...
                    is_active_fn,
                    &[],
                    "top_level_val_init_effect_is_active",
                )?
                .try_as_basic_value()
                .basic()
                .ok_or(LlvmEmitError::UnsupportedMainBody {
...
fn closure_callee_resume_entry_fn_name(closure_id: hir::ClosureId) -> String {
