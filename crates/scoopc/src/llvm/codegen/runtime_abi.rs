...
impl<'a, 'ctx> MainCodegen<'a, 'ctx> {
    ...
    pub(super) fn declare_runtime_task_create(&self) -> FunctionValue<'ctx> {
        ...
        // `void* scoop_task_create(void* (*body_fn)(void* closure_obj), void* closure_obj)`
        let gc_i8_ptr_ty = self.llvm_gc_i8_ptr_type();
    ...
    pub(super) fn declare_runtime_task_poll(&self) -> FunctionValue<'ctx> {
        const NAME: &str = runtime_symbols::SCOOP_TASK_POLL;
        if let Some(existing) = self.module.get_function(NAME) {
            return existing;
        }
        ...
        let gc_i8_ptr_ty = self.llvm_gc_i8_ptr_type();
        let i32_ty = self.context.i32_type();
        let i64_slot_ty = self.context.ptr_type(AddressSpace::default());
        let out_gc_ref_slot_ty = self.context.ptr_type(AddressSpace::default());
        let param_tys: [BasicMetadataTypeEnum<'ctx>; 3] = [
            gc_i8_ptr_ty.into(),
            i64_slot_ty.into(),
            out_gc_ref_slot_ty.into(),
        ];
        let fn_ty = i32_ty.fn_type(&param_tys, false);
        self.module.add_function(NAME, fn_ty, None)
    }

    pub(super) fn declare_runtime_task_step_ready(&self) -> FunctionValue<'ctx> {
        const NAME: &str = runtime_symbols::SCOOP_TASK_STEP_READY;
        if let Some(existing) = self.module.get_function(NAME) {
            return existing;
        }
    ...
    pub(super) fn declare_runtime_task_step_pending(&self) -> FunctionValue<'ctx> {
        const NAME: &str = runtime_symbols::SCOOP_TASK_STEP_PENDING;
        if let Some(existing) = self.module.get_function(NAME) {
            return existing;
        }
