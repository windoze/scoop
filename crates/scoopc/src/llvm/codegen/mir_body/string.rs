...
impl<'a, 'ctx> MainCodegen<'a, 'ctx> {
    pub(in crate::llvm::codegen) fn codegen_mir_unresolved_name_with_source_ty(
        ...
        let source_cg = self.cg_ty_of_mir_type(source_types, source_ty).unwrap_or_else(|| {
            panic!("codegen_mir_unresolved_name_with_source_ty: MIR verifier accepted unsupported source type")
        });
