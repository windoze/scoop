...
impl<'cg, 'a, 'ctx> CallableEmitter<'cg, 'a, 'ctx> {
    pub(super) fn store_no_outward_call_complete(
        ...
        let step_layout = self.abi.step_layout(step_schema).ok_or_else(|| {
            frontend_error(format!(
                "dynamic call site {} 缺少 return step schema s{} layout",
                site_id.as_u32(),
                step_schema.as_u32()
            ))
        })?;
        if !step_layout.cases().is_empty() {
            return Err(frontend_error(format!(
                "source-slice dynamic call site {} return step schema s{} 含 outward case，必须走 boundary lowering",
                site_id.as_u32(),
                step_schema.as_u32()
            )));
        }
        ...
        match payload {
            ...
            None => {
                ...
                if slot.cg_ty != CgTy::Unit && slot.cg_ty != CgTy::Never {
                    return Err(frontend_error(format!(
                        "dynamic call site {} non-Unit target 缺少 Complete payload",
                        site_id.as_u32()
                    )));
                }
    ...
    pub(super) fn create_continuation_object_with_state_tag(
        ...
        {
            return Err(frontend_error(format!(
                "continuation composition resume_state 漂移：object={:?} contract=st{}",
                resume_state,
        ...
        let owner_step = self
            ...
            .ok_or_else(|| {
                frontend_error(format!(
                    "callable `{}` 缺少 owner step schema s{}",
        ...
        let continuation_case = owner_step
            ...
            .ok_or_else(|| {
                frontend_error(format!(
                    "callable `{}` step schema s{} 缺少 continuation case c{}",
                    self.callable.root_fqn(),
                    self.abi_step_schema.as_u32(),
                    case_tag.as_u32()
                ))
        ...
        let target = match dispatch
            ...
            None => {
                return Err(frontend_error(format!(
                    "callable `{}` case c{} continuation schema k{} 缺少 owner continuation drive target",
                    self.callable.root_fqn(),
                    case_tag.as_u32(),
                    continuation_schema.as_u32(),
                )));
        ...
        let step_fun = self.codegen.continuation_step_function(target);
        let cont_layout = self
            .abi
            .continuation_layout(self.callable.continuation_object())
            .ok_or_else(|| {
                frontend_error(format!(
                    "callable `{}` 缺少 continuation object ko{} layout",
                    self.callable.root_fqn(),
                    self.callable.continuation_object().as_u32()
                ))
            })?;
        let cont_ptr = self.codegen.alloc_gc_struct(
            self.mir_fun.span,
            cont_layout.llvm_ty(),
    ...
    pub(super) fn load_captured_callee_suspend_state_ref(
        ...
        let gep = self.codegen.builder.build_struct_gep(
            ...
            cont_ptr,
            CONT_FIELD_CAPTURED_CALLEE_SUSPEND_STATE,
            "captured_callee_suspend_state_gep",
        )?;
        Ok(self
            .codegen
            .builder
