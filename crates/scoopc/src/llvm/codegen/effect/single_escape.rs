impl<'a, 'ctx> MainCodegen<'a, 'ctx> {
    pub(super) fn codegen_handle_expr_unified_single_escape_continuation_leaf<'hir>(
        &mut self,
            ...
            if !indirect_sites.is_empty() {
                if indirect_sites.len() > 1 {
                    return Err(LlvmEmitError::UnsupportedMainBody {
                        kind: "handle escape continuation indirect site (multiple call sites not yet supported)",
                        at: arm.span.into(),
                    });
                }
