impl<'a, 'ctx> MainCodegen<'a, 'ctx> {
    ...
    fn collect_mixed_escape_used_between_while_sites<'hir>(
        ...
        {
            return Err(LlvmEmitError::UnsupportedMainBody {
                kind: "handle mixed-arm escape continuation (only same-body-stmt or top-level separate-stmt direct / indirect coexistence in while body supported)",
                at: next_site.decl.span.into(),
            });
        }
        ...
        if current_path.len() == 1
            && next_path.len() == 1
            && Self::mixed_escape_while_separate_stmt_order_supported(current_path, next_path)
        {
    ...
    fn codegen_mixed_escape_matrix_prefix_from_stmts<'hir>(
        ...
        for (idx, stmt) in stmts.iter().enumerate() {
            ...
            match &site.resume_path[depth + 1] {
                ...
                MixedEscapeDirectFrame::WhileBody { .. } => {
                    return Err(LlvmEmitError::UnsupportedMainBody {
                        kind: "handle mixed-arm escape continuation (nested while path after block not yet supported)",
                        at: expr.span.into(),
                    });
                }
            }
            ...
            FnMut(&mut Self, usize, &MixedEscapeIndirectSite<'hir>) -> Result<(), LlvmEmitError>,
    {
        ...
        for (idx, stmt) in block.stmts.iter().enumerate().skip(start_idx) {
            ...
            if block_level + 1 == next_path.len() {
                let expected_stmt_kind = match &next_site.kind {
                    MatrixEscapeSiteKind::Direct { .. } => {
                        "handle mixed-arm escape continuation (expected perform binding)"
                    }
                    MatrixEscapeSiteKind::Indirect { .. } => {
                        "handle mixed-arm escape continuation (expected indirect call binding)"
                    }
                };
                let hir::StmtKind::Val(decl) = &stmt.kind else {
                    return Err(LlvmEmitError::UnsupportedMainBody {
                        kind: expected_stmt_kind,
                        at: stmt.span.into(),
                    });
                };
                if !std::ptr::eq(decl, next_site.decl) {
            ...
            match next_path.get(block_level + 1) {
                Some(MixedEscapeDirectFrame::Block {
                    block: expected_next_block,
                    ..
                }) => {
                ...
                Some(MixedEscapeDirectFrame::IfThen { .. } | MixedEscapeDirectFrame::IfElse { .. }) => {
                    match &next_site.kind {
                        MatrixEscapeSiteKind::Direct { site } => {
                            let local_sites = [MatrixEscapeSite {
                                stmt_idx: 0,
                                decl: site.decl,
                                id: site.id,
                                kind: MatrixEscapeSiteKind::Direct {
                                    site: MixedEscapeDirectSite {
                                        ...
                                        args: site.args,
                                        id: site.id,
                                        resume_path: site.resume_path[block_level + 1..].to_vec(),
                                    },
                                },
                            }];
                            self.codegen_mixed_escape_matrix_if_stmt_direct_sites(
                                ...
                                &local_sites,
                                body_lift_ids,
                                |cg, _local_pc, _local_site| emit_direct(cg, next_pc, site),
                            )?;
                        }
                        MatrixEscapeSiteKind::Indirect { site } => {
                            ...
                            self.codegen_mixed_escape_matrix_if_stmt_indirect_sites(
                                ...
                                &local_sites,
                                body_lift_ids,
                                |cg, _local_pc, _local_site| emit_indirect(cg, next_pc, site),
                            )?;
                        }
                    }
                    if self
                ...
                None => unreachable!("validated non-terminal block path"),
            }
        }

        if entered_target_frame {
            Ok(())
        } else {
            Err(LlvmEmitError::UnsupportedMainBody {
                kind: "handle mixed-arm escape continuation (perform site missing)",
            ...
            FnMut(&mut Self, usize, &MixedEscapeIndirectSite<'hir>) -> Result<(), LlvmEmitError>,
    {
        ...
        if Self::mixed_escape_while_same_stmt_mixed_path_supported(current_path, next_path) {
            ...
            return match nested_current.resume_path.first() {
                ...
                _ => Err(LlvmEmitError::UnsupportedMainBody {
                    kind: "handle mixed-arm escape continuation (only same-body-stmt or separate-stmt direct / indirect coexistence in while body supported)",
                    at: next_site.decl.span.into(),
                }),
            };
        }
        ...
        F: FnMut(&mut Self, usize, &MixedEscapeDirectSite<'hir>) -> Result<(), LlvmEmitError>,
    {
        ...
        match site.resume_path[1] {
            MixedEscapeDirectFrame::Block {
                ...
                let nested_escape_site = MatrixEscapeSite {
                    stmt_idx: 0,
                    decl: site.decl,
                    id: site.id,
                    kind: MatrixEscapeSiteKind::Direct {
                ...
                self.env.push_scope();
                let mut emit_nested_direct =
                    |cg: &mut Self,
                     _nested_pc: usize,
                     _nested_site: &MixedEscapeDirectSite<'hir>|
                     emit_intercept(cg, site_pc, site);
                let mut unexpected_indirect =
                    |_cg: &mut Self,
                     _nested_pc: usize,
                     nested_site: &MixedEscapeIndirectSite<'hir>|
                     -> Result<(), LlvmEmitError> {
                        Err(LlvmEmitError::UnsupportedMainBody {
                            kind: "handle mixed-arm escape continuation (unexpected indirect site while scanning direct while site)",
                            at: nested_site.decl.span.into(),
                        })
                    };
                self.codegen_mixed_escape_matrix_scan_block_to_site_from_idx(
                    0,
        ...
        match site.resume_path[1] {
            MixedEscapeDirectFrame::Block {
                ...
                let mut unexpected_direct =
                    |_cg: &mut Self,
                     _nested_pc: usize,
                     nested_site: &MixedEscapeDirectSite<'hir>|
                     -> Result<(), LlvmEmitError> {
                        Err(LlvmEmitError::UnsupportedMainBody {
                            kind: "handle mixed-arm escape continuation (unexpected direct site while scanning indirect while site)",
                            at: nested_site.decl.span.into(),
                        })
                    };
                let mut emit_nested_indirect =
                    |cg: &mut Self,
                     _nested_pc: usize,
                     _nested_site: &MixedEscapeIndirectSite<'hir>|
                     emit_site(cg, site_pc, site);
                self.codegen_mixed_escape_matrix_scan_block_to_site_from_idx(
                    0,
                    block,
                    0,
        ...
        F: FnMut(&mut Self, usize, &MixedEscapeIndirectSite<'hir>) -> Result<(), LlvmEmitError>,
    {
        ...
        for (idx, body_stmt) in body.stmts.iter().enumerate() {
            ...
            {
                if Self::mixed_escape_block_only_path_supported(&site.resume_path[1..]) {
                    self.codegen_mixed_escape_matrix_nested_tail_after_indirect_site_from_depth(
                        site,
                        1,
                        body_lift_ids,
            ...
            FnMut(&mut Self, usize, &MixedEscapeIndirectSite<'hir>) -> Result<(), LlvmEmitError>,
    {
        ...
        if Self::mixed_escape_while_separate_stmt_order_supported(
            ...
            return self.codegen_mixed_escape_matrix_while_stmt_indirect_site(
                stmt,
                indirect_pc,
                indirect_site,
                body_lift_ids,
                |cg, site_pc, site| emit_indirect(cg, site_pc, site),
            );
        }
    ...
    fn codegen_mixed_escape_matrix_while_tail_after_mixed_direct_site<'hir, F>(
        &mut self,
        current_direct: &MixedEscapeDirectSite<'hir>,
        future_indirect_pc: usize,
        future_indirect: &MixedEscapeIndirectSite<'hir>,
        body_lift_ids: &HashSet<hir::SymbolId>,
        mut emit_intercept: F,
    ) -> Result<(), LlvmEmitError>
    where
        F: FnMut(&mut Self, usize, &MixedEscapeIndirectSite<'hir>) -> Result<(), LlvmEmitError>,
    {
        let Some(first_frame) = current_direct.resume_path.first() else {
            return Err(LlvmEmitError::UnsupportedMainBody {
                kind: "handle mixed-arm escape continuation (missing while path)",
                at: current_direct.decl.span.into(),
            });
        };
        let MixedEscapeDirectFrame::WhileBody {
            while_cond,
        ...
        let Some(MixedEscapeDirectFrame::WhileBody {
            while_cond: future_while_cond,
            while_body: future_while_body,
            stmt_idx: future_indirect_stmt_idx,
        }) = future_indirect.resume_path.first()
        else {
            return Err(LlvmEmitError::UnsupportedMainBody {
                kind: "handle mixed-arm escape continuation (expected while site)",
                at: future_indirect.decl.span.into(),
            });
        };
        if !std::ptr::eq(*while_cond, *future_while_cond)
            || !std::ptr::eq(*while_body, *future_while_body)
        ...
        if !Self::mixed_escape_while_same_stmt_mixed_path_supported(
            &current_direct.resume_path,
            &future_indirect.resume_path,
        ) && !Self::mixed_escape_while_separate_stmt_order_supported(
            &future_indirect.resume_path,
            &current_direct.resume_path,
        ) {
        ...
        if !Self::mixed_escape_while_nested_path_supported(&current_direct.resume_path) {
            return Err(LlvmEmitError::UnsupportedMainBody {
                kind: "handle mixed-arm escape continuation (deeper nested direct site in while body not yet supported)",
                at: current_direct.decl.span.into(),
            });
        ...
        // 当前 direct 完成后，future iteration 需要回到源码顺序里更早的 indirect site。
        self.env.push_scope();
        if current_direct.resume_path.len() > 1 {
            self.codegen_mixed_escape_matrix_nested_tail_after_site_from_depth(
                current_direct,
                1,
                body_lift_ids,
        ...
        for (idx, body_stmt) in while_body.stmts.iter().enumerate() {
            if idx < *future_indirect_stmt_idx {
                self.codegen_mixed_escape_matrix_replay_stmt(body_stmt, body_lift_ids)?;
                continue;
            }
            ...
            if let Some(bb) = self.builder.get_insert_block()
                && bb.get_terminator().is_none()
            {
                if Self::mixed_escape_block_only_path_supported(&future_indirect.resume_path[1..])
                {
                    self.codegen_mixed_escape_matrix_nested_tail_after_indirect_site_from_depth(
                        future_indirect,
                        1,
                ...
                for body_stmt in while_body.stmts.iter().skip(*future_indirect_stmt_idx + 1) {
                    self.codegen_mixed_escape_matrix_replay_stmt(body_stmt, body_lift_ids)?;
                }
                self.env.pop_scope();
                self.builder.build_unconditional_branch(cond_bb)?;
    ...
    fn codegen_handle_expr_immediate_resume_with_escape_sibling_site_matrix<'hir>(
        ...
        for (stmt_idx, site_pcs) in &escape_site_pcs_by_stmt_idx {
            ...
            if !while_direct_sites.is_empty() {
                if !while_indirect_sites.is_empty() {
                    ...
                    let supports_same_stmt =
                        Self::mixed_escape_while_same_stmt_mixed_path_supported(
                            &direct_site.resume_path,
                            &indirect_site.resume_path,
                        );
                    let supports_top_level_separate_stmt =
                        direct_site.resume_path.len() == 1
                            && indirect_site.resume_path.len() == 1
                            && (Self::mixed_escape_while_separate_stmt_order_supported(
                                &direct_site.resume_path,
                                &indirect_site.resume_path,
                            ) || Self::mixed_escape_while_separate_stmt_order_supported(
                    ...
                    if !supports_same_stmt && !supports_top_level_separate_stmt {
                        return Err(LlvmEmitError::UnsupportedMainBody {
                            kind: "handle mixed-arm escape continuation (only same-body-stmt or top-level separate-stmt direct / indirect coexistence in while body supported)",
                            at: handle.body.stmts[*stmt_idx].span.into(),
                        });
            ...
            for (site_pc, state_bb) in state_bbs.iter().enumerate() {
                ...
                match &site.kind {
                    MatrixEscapeSiteKind::Direct { .. } => {
                        ...
                        if let MatrixEscapeSiteKind::Direct { site: direct_site } = &site.kind {
                            if matches!(
                                ...
                                } else if let Some(prev_pc) =
                                    while_prev_site_pc_by_pc.get(&site_pc).copied()
                                {
                                    let MatrixEscapeSiteKind::Indirect {
                                        ...
                                        return Err(LlvmEmitError::UnsupportedMainBody {
                                            kind: "handle mixed-arm escape continuation (expected previous indirect site)",
                                            at: direct_site.decl.span.into(),
                                        });
                                    };
                                        ...
                                        |cg, reenter_pc, future_indirect_site| {
                                                ...
                                                outer_visible_supported.iter().enumerate()
                                            {
                                                ...
                                                let local = cg.env.get(cap.id).ok_or(
                                                    LlvmEmitError::UnsupportedMainBody {
                                                        kind: "mixed escape capture local not found",
                                                        at: future_indirect_site.decl.span.into(),
                                                    },
                                                )?;
                                                if local.ty != cap.ty {
                                                    return Err(LlvmEmitError::UnsupportedMainBody {
                    ...
                    MatrixEscapeSiteKind::Indirect {
                        ...
                        } else if while_next_site_pc_by_pc.contains_key(&site_pc)
                            && matches!(
        ...
        let resume_blocks =
            self.build_mixed_escape_resume_blocks(func, "handle_mixed_escape_matrix");
