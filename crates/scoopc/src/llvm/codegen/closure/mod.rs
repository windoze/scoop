...
impl<'a, 'ctx> MainCodegen<'a, 'ctx> {
    ...
    pub(in crate::llvm::codegen) fn codegen_closure_expr(
            ...
            self.module.get_function(&closure_identity.body_symbol)
        {
            ...
            let ret_cg = self.cg_ty_of_type_id(fun_ty.return_ty, "lambda return type");
        ...
        let env_i8 = if captures.is_empty() {
            ...
            for (idx, (id, name, ty_id, _mutable)) in capture_bindings.iter().enumerate() {
                ...
                let cg_ty = self.cg_ty_of_type_id(*ty_id, "closure capture local type");
    ...
    fn codegen_closure_fun_body(
        ...
        let declared_return_cg = self.cg_ty_of_type_id(fun_ty.return_ty, "lambda return type");
        ...
        if !spec.capture_bindings.is_empty() {
            ...
            for (idx, (id, name, ty_id, mutable)) in spec.capture_bindings.iter().enumerate() {
                let target_ty = self.cg_ty_of_type_id(*ty_id, "closure capture type");
    ...
    fn llvm_closure_env_type(
        ...
        for (_id, _name, ty_id, _mutable) in capture_bindings {
            let cg_ty = self.cg_ty_of_type_id(*ty_id, "closure env capture field type");
