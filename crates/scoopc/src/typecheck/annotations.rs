...
//! - `@Extern/@Intrinsic/@NoGC/@Unsafe` 等内建注解的特殊行为（见 TODO T1003+）。
...
pub fn check_file_annotations(
    ...
    for item in &file.items {
        match item {
            ast::Item::TypeAlias(ta) => check_annotation_uses(source, file, index, env, &ta.annotations)?,
            ast::Item::Fun(fun) => check_annotation_uses(source, file, index, env, &fun.annotations)?,
            ast::Item::ExtensionProperty(p) => {
                check_annotation_uses(source, file, index, env, &p.annotations)?;
            }
            ast::Item::Val(v) => check_annotation_uses(source, file, index, env, &v.annotations)?,
            ast::Item::Type(ty) => check_type_decl_annotations(source, file, index, env, ty, &pkg_prefix)?,
            ast::Item::Object(obj) => {
