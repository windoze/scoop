...
/// - `compilable_files`：与 `compilable_source_paths` 一一对应的预解析 SysrootFile 缓存，
///   供 single-file dump 管线（HIR/MIR/effects 等）扩充 `Index` 时使用——
///   full build 仍走 `frontend::load_stdlib_sources` 重新加载，不依赖该缓存。
#[derive(Debug)]
pub struct Sysroot {
    pub root: PathBuf,
    ...
    /// 预解析的 compilable sysroot 源文件，用于 dump-mode 管线把内建类型（如
    /// `Option`/`Map` 等）从 overlay `core.scoop` 注入到 single-file Index。
    ...
    pub fn load_from_with_overlay(
        ...
        for path in paths {
            ...
            if is_compilable_sysroot_file(&path, &source, &ast) {
                ...
                compilable_files.push(SysrootFile { path, source, ast });
                continue;
            }
