...
- 当前状态：`T4003S` / `T4003SR` / `T4003T` / `T4003TR` / `T4004a1` / `T4004a2` / `T4004b` / `T4004R` / `T4005` / `T4005R` / `T4005S` / `T4005T` 已完成；普通顶层 `val` 现已具备独立 once-init + 稳定读取主线，局部 `val` destructuring 也已确认通过“synthetic subject + 命名 binder + 通用投影/校验 helper”落入统一可执行主线，可直接被顶层 once-init 版本复用。顶层 pattern binding 现已同时覆盖“显式整体类型注解”“initializer 驱动推断 + 跨文件 binder 类型可见性”“HIR / LLVM once-init lowering”“state-machine hidden boundary active/inactive 处理”四条路径；Elvis `?:` 复审也已确认通过“lhs inner type 驱动 rhs expected-context + `when` arm closure codegen 直连闭包主线”的方式真正变成稳定可执行特性。当前顶层命名 `val`、顶层 pattern binder 与顶层 `FunPtr` 也已经统一进入 callable-value call 主线；顺带修掉的 tuple-literal closure expected-context 裂缝则让这条路径在“pattern binder hidden subject = tuple literal”场景下不再额外分叉。P3 阶段当前仅剩 `T4005SR` 复审，下一项切换为 `T4005SR`。
