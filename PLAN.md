...
- [ ] Token 集：关键字、标识符、数字、字符串、运算符、注解、泛型尖括号等
- [ ] 注释：行注释 `//`、块注释 `/* */`（允许嵌套与否需按 spec 决定）
- [ ] 字符串：
  - 普通字符串
  - `f` 插值字符串：`f"hello {x}"`
  - raw 三引号字符串：`""" ... """` 与 `f""" ... """`
  - 大括号转义：`f"use {{braces}}"` → 输出 `{braces}`
- [ ] Span（源代码位置）必须贯穿全流程，用于 diagnostics 与 fixtures 对齐
...
- [ ] 两阶段/多阶段解析：
  ...
  - 再解析函数体与初始化表达式
- [ ] 处理重名：按 Kotlin-like 规则给出清晰错误
- [ ] 解析 `this`、扩展函数 receiver、成员/顶层同名优先级
