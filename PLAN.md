...
- 落地顺序（T1606 已拆分为子任务 T1606a~T1606g；T1608/T1607 前置）：
  ...
  - T1606e（DONE）：handle body 任意控制流（while 循环/if-else 分支）显式验证 — codegen 解除 `while` 语句在 escape continuation body 中的限制；新增 3 个 run-pass fixtures（while-in-body、while-accumulate-string、control-flow-mix），覆盖 while 在 perform 前/间/后三种路径 + 混合 Int/String 类型 + if/else+while 组合；全部在 `SCOOP_GC_STRESS=1` 下稳定
  - T1606f~g：间接 perform/嵌套 handle 的显式验证
