...
- PENDING（T0147c-2b）：Clippy 基线清理（typecheck 支撑模块签名收口）——处理 `annotations.rs` / `override_effects.rs` / `properties.rs` / `val_pat.rs` 中 12 个 `too_many_arguments`，先清外围模块。
- PENDING（T0147c-2c）：Clippy 基线清理（typecheck expr 中等规模路径签名收口）——处理 `member.rs` / `ops.rs` / `stmt.rs` 中 17 个 `too_many_arguments`，沿调用链下沉共享上下文。
- PENDING（T0147c-2d）：Clippy 基线清理（typecheck expr 主干签名收口）——处理 `call.rs` / `entry.rs` / `infer.rs` 中剩余 36 个 `too_many_arguments`，完成后该类 clippy 告警应全部清零。
