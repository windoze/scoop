...
- TODO（T0146c2）：Char sysroot/runtime 文本化——在 `T0146c1` 稳定后，再补 `sysroot/core.scoop` 的 `struct Char : Hashable, ToString`、`toInt()/toString()/hash()` 声明、runtime `scoop_char_to_string`、where-bound `ToString` / `print` 路径，以及多文件回归。
