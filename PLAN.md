...
- 进展更新（2026-05-01）：`T5001f8a0` / `T5001f8a0R` 已完成。已复核 state/arm body 对 outer mutable local 的读写入口会在 use-site 通过 `rematerialize_ptr_in_current_block(...)` 重建指针链，避免依赖 stale heap-slot pointer；定向验证 `continuation_resume_enum.scoop`（GC env）与 runtime_gc fixture `effect_outer_mutable_state_body_writeback_basic.scoop` 均通过。下一步继续推进 `T5001f8a`。
