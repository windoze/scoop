...
- TODO（T0146b）：Char 类型静态语义——补齐 `ValueTypeKind::Char` / `BuiltinTypes::char_`、HIR Char literal/pattern、typecheck 比较/`when` pattern、comptime `ConstValue::Char` 与 `toInt()`。
- TODO（T0146c）：Char 运行期与后端——补齐 sysroot `Char`、runtime `scoop_char_to_string`、LLVM `CgTy::Char` / literal / compare / method dispatch / `when` pattern codegen，以及 run-pass / 多文件回归。
