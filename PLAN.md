...
- 2026-04-02：修复 LLVM codegen 的 escape continuation 捕获/恢复：continuation state 允许捕获 `Bool/Int`（word-sized handle，如 `Task<T>`/`Executor`），避免 step trampoline 中引用外层变量时报 `unknown local value`；同时 stdlib `Task.andThen` 以“两段 handle”规避 v0 后端“单个 perform 点”的限制。
