...
- PENDING（T0147c-3b）：Clippy 基线清理（收缩 `ExprTypeError` 主路径）——第二轮统一处理 `typecheck::expr/**`、`eff_row_subst`、`val_pat`、`when_*` 上的 `result_large_err`，避免在零散模块里各自引入不同约定。
- PENDING（T0147c-3c）：Clippy 基线清理（清零结构性 warning）——在大 `Err` 问题收口后，再清理 `private_interfaces` / `dead_code` / `large_enum_variant` / `type_complexity` / `question_mark` / `if_same_then_else` / `while_let_loop` 等剩余 lint，恢复严格 gate。
- PENDING（T0147c-3）：Clippy 基线清理（`result_large_err` + 零散 warning）——已拆分为 `T0147c-3a ~ T0147c-3c` 顺序落地，最终目标不变：恢复 `cargo clippy --workspace --all-targets -- -D warnings` 全量通过。
