...
> 落地记录（2026-08-16）：(1) MirDecls 声明段定稿进 MIR archive（28 查询点
> 重接线，`scoop2_lir` 摘除 `scoop2_hir`——ty 经 `scoop2_mir` re-export）；
> (2) slot 0 占位 + backfill_call_sites 删除（DispatchSlotTables map 期定稿）；
