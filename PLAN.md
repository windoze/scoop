...
- 当前状态（T5001f7，2026-04-30）：已定位并修复 `Continuation.resume` consumed-root verify-roots 回归的真实根因。问题不在 `when_subject`、payload boxing 或 receiver reload 本身，而在 runtime continuation ABI 对 `k->state` 的长期 ownership：`ScoopContinuation.state` 会在未来 resume 时被 `step/dispatch` 以 raw C pointer 形态重新传回 effect frame，但此前只在 `scoop_continuation_alloc(...)` 调用栈里做短暂 pin，后续 GC 仍可搬迁该 effect frame。这样 `continuation_resume_struct_with_ref.scoop` 的 resumed body 进入 `scoop.effect.dispatch/step.35d0289` 时，就会把 stale `k->state` 原样写进两层 explicit frame slot0，并在 `println(n.name)` 触发的 verify-roots 窗口里报 invalid root。现已把 continuation state 与 continuation 生命周期绑定为 pinned：`runtime/c/scoop_runtime.c` 的 `scoop_continuation_alloc(...)` 在写入 `k->state` 后保留一个长期 pin，`scoop_continuation_release(...)` 在 continuation 释放时对称 `unpin`。这样 future resume 不再依赖 moving update 去修复 raw `state` owner。定向验证已通过 `cargo build -p scoop_runtime`、`env SCOOP_GC_MOVE=1 SCOOP_GC_STRESS=1 SCOOP_GC_VERIFY_ROOTS=1 cargo run -p scoop -- run tests/fixtures/run-pass/continuation_resume_struct_with_ref.scoop`、`cargo run -p scoop -- test --fixtures tests/fixtures/run-pass/continuation_resume_struct_with_ref.scoop` 与 `cargo clippy --all-targets -- -D warnings`；继续执行 `cargo run -p scoop -- test` 时，suite 已越过该 fixture。
- 新阻塞（2026-04-30）：在 `T5001f7` 修复后继续执行 `cargo run -p scoop -- test`，新的首个失败切换为 `tests/fixtures/run-pass/class_init_raise_cleanup_init_block_gc_basic.scoop`。当前程序输出 `3`，而 golden 期望 `0`，说明 class init `Raise.raise(...)` unwind 后仍有失败构造对象或 ctor-call 临时 roots 被错误保留。按同一规则，现已新增 `T5001f8/T5001f8R` 作为 `T5001f7R` 之前的前置任务，再继续 review / 全量验收。
