...
- 进展更新（2026-05-01）：`T5001f8a` 已完成。已在默认环境与 `SCOOP_GC_MOVE=1 SCOOP_GC_STRESS=1 SCOOP_GC_VERIFY_ROOTS=1` 下复验 `continuation_resume_enum.scoop`，并确认最小回归 `effect_escape_continuation_outer_mutable_writeback_basic.scoop` 覆盖“handle-return 后立刻读回”的窗口；同时 `cargo test --all` 与 `cargo clippy --all-targets -- -D warnings` 均通过。下一步进入 `T5001f8aR` review。
