...
- 2026-04-02：将 T1327（类初始化兼容：复杂继承链与 effect 细节）拆分为 T1327a～T1327c，以保持“可单独实现 & 单独验证”的粒度；本轮先落地 T1327a（单继承链初始化顺序 + 字段 layout 前缀），后续再补齐 init 期间 Raise/effect 传播的 cleanup 与 super ctor 实参解析/secondary ctor delegation。
