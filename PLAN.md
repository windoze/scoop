...
- 2026-04-29：`T5000j3 扩展更多 higher-order / closure / object-init / top-level-init 场景到 production MIR 主线` 已判定为单轮过大任务，现拆成 `T5000j3a`～`T5000j3b` 两个实现子任务与对应 review。
  - 拆分依据：
    - `crates/scoopc/src/llvm/emit.rs` / `reachability.rs` 中 raw non-generic canonical body 选择当前仍被 `raw_non_generic_pattern_candidate(...)` 限制为 pattern candidate，而 `crates/scoopc/src/llvm/codegen/mir_body.rs` 实际已经支持更宽的一批 raw MIR 形状，其中包括 `TopLevelRef` 驱动的 `top_level_consts`、`top_level_immutable_values`、`object_inits` 与 `top_level_vars`；
    - 与之相对，`CallKind::Closure` / `CallKind::FunValue`、`MakeClosure`、`CaptureBox*` 目前在 MIR bridge 中仍显式返回 unsupported，higher-order / closure 仍需单独处理，不适合与 init 场景混在同一轮；
    - 因此第一刀应先把“已有 lowering / reachability 支持但尚未被 canonical 选择放开”的 init 场景接入 production MIR 主线，再继续处理 higher-order / closure。
  - 拆分顺序：
    - `T5000j3a`：先扩展 `top-level-init / object-init` 相关 raw non-generic body 到 production MIR 主线；
    - `T5000j3b`：再扩展更多 higher-order / closure 场景到 production MIR 主线。
  - 下一条待执行任务切换为 `T5000j3a 扩展 top-level-init / object-init 相关 raw non-generic body 到 production MIR 主线`。
- 2026-04-29：`T5000j3a 扩展 top-level-init / object-init 相关 raw non-generic body 到 production MIR 主线` 已完成。
  - 实现结果：
    - `crates/scoopc/src/llvm/codegen/mod.rs` 现为 caller-side raw non-generic body 提供按作用域收口的 canonical 选择 helper：继续保留既有 pattern body，并新增通过 `TopLevelRef` 访问 `top_level_consts` / `top_level_immutable_values` / `top_level_vars` / `object_inits` 的 init 相关 body；普通 arithmetic/direct-call helper 不再因为“所有 raw non-generic body 都可进 canonical 选择”而误切到 production raw MIR bridge；
    - `crates/scoopc/src/llvm/emit.rs` 与 `crates/scoopc/src/llvm/reachability.rs` 已统一改经这条 candidate 选择边界；`ReachabilityInputs` 同步补入 `top_level_vars` 与 `object_inits`，让 init 相关 body 的 canonical body 选择与 canonical reachability 扫描保持一致；
    - `crates/scoopc/src/llvm/codegen/mir_body.rs` 现明确把 `Return { value: None }` 视为 raw MIR unsupported。generic MIR 仍以隐式尾表达式约定保留返回值时，production bridge 不再把它错降成类型默认值，而是保守退回 HIR-compatible body；本轮排查中暴露的 `keepLooping` / `fun_call_add_basic` 类误发射因此消失；
    - `crates/scoopc/src/llvm/tests.rs` 新增并整理了 `top-level immutable init`、object value init、closure fallback、implicit tail-return fallback、non-init/non-pattern helper fallback 与 ctor-call `Todo` reachability fallback 回归，覆盖 `j3a` 目标场景与新增边界保护。
  - 阶段结论：
    - `j3a` 的 production 覆盖现在只是在 canonical materialized MIR 主线上放宽到 init 相关 raw body，没有把普通 helper、higher-order shape 或隐式 tail-return body 一并倒灌进 backend 现场推断；
    - 本轮验证中未发现需要插入到 `T5000j3aR` 之前的新前置缺陷任务。
  - 验证结果：
    - `cargo test -p scoopc production_codegen_ -- --nocapture`
    - `cargo test --all`
    - `cargo clippy --all-targets -- -D warnings`
...
- 2026-04-29：`T5000j3bR Review：确认 higher-order / closure 场景扩张没有把分析责任倒灌回 backend` 已完成。
  - 复核结论：
    - 已复核 `crates/scoopc/src/llvm/codegen/mir_body.rs`，确认 production 主线新增的 higher-order / closure 覆盖继续只消费 canonical materialized MIR body、`MaterializedMirPassView`、`mir_types -> codegen TypeStore` 的等价类型映射，以及 `known_fun_body_may_outward_effect(...)` 提供的 shared suspendability 事实；backend 仍只负责 closure object / capture-box lowering、function-value indirect-call ABI 组装与 effect-boundary 安装，没有重新承担 target-set 缩减、escape 推断或 provenance 分析职责；
    - 已复核 `crates/scoopc/src/llvm/reachability.rs` 与 `crates/scoopc/src/llvm/codegen/mod.rs`，确认 raw non-generic candidate / MIR-compatible scan 只是在 canonical materialized MIR 主线上放宽 `MakeTuple` / `TupleGet` / `MakeClosure` / `CaptureBox*` / `ClosureCall` / `FunValueCall` 等已收口形状；`Virtual` / `Interface` / `Resume`、effect/handle、`Return { value: None }` 等未收口 higher-order / control-transfer 形状仍继续保留在 HIR-compatible fallback，没有把剩余分析责任倒灌回 backend；
    - 已复核 `crates/scoopc/src/llvm/codegen/effect/state_machine_bridge.rs`、`crates/scoopc/src/effect/analysis.rs` 与 `crates/scoopc/src/mir/pass_view.rs`，确认 effect/suspendability/continuation-escape 相关输入继续来自 shared `collect_known_fun_call_suspendability(...)`、`EffectAnalysisCtx`、`ProgramFacts` 与 pass-view escape facts；LLVM lowering 侧只消费这些 shared facts，不在现场重算 higher-order/effect 分析；
  - 验证结果：
    - `cargo fmt --all --check`
    - `cargo test -p scoopc production_codegen_lowers_raw_mir_non_capturing_closure_body -- --nocapture`
    - `cargo test -p scoopc production_codegen_lowers_raw_mir_mutable_capture_closure_body -- --nocapture`
    ...
    - `cargo run -p scoop -- test`
    - 全部通过。
  - review 结论：higher-order / closure 场景扩张仍建立在 materialized MIR、pass artifacts 与 shared effect facts 之上；当前未发现需要插入到 `T5000j3R` 之前的新前置缺陷任务。
  - 下一条待执行任务切换为 `T5000j3R Review：确认 higher-order / init 场景扩张没有把分析责任倒灌回 backend`。
- 2026-04-29：`T5000j3R Review：确认 higher-order / init 场景扩张没有把分析责任倒灌回 backend` 已完成。
  - 复核结论：
    - 已复核 `crates/scoopc/src/llvm/emit.rs`、`crates/scoopc/src/llvm/codegen/mod.rs` 与 `crates/scoopc/src/llvm/reachability.rs`，确认 production 主线路径只是在 canonical materialized callable body / raw non-generic candidate / MIR-compatible scan 边界上放宽 `top-level-init`、`object-init`、`MakeClosure`、`CaptureBox*`、`ClosureCall`、`FunValueCall` 与 pattern 相关已收口形状；`Virtual` / `Interface` / `Resume`、effect/handle、`Return { value: None }` 等未收口形状仍继续经 HIR-compatible fallback 处理，没有把 target-set 收缩或结构分析长回 backend；
    - 已复核 `crates/scoopc/src/llvm/codegen/mir_body.rs` 与 `crates/scoopc/src/llvm/codegen/object_init.rs`，确认 top-level/object init 访问、closure object / capture-box lowering 与 function-value indirect-call 继续只消费 canonical materialized MIR body、`MaterializedMirPassView`、`mir_types -> codegen TypeStore` 等价类型映射，以及既有 object-init / top-level-init side tables；backend 保留的是 lowering、ABI 组装与显式 outcome boundary 安装，而不是现场重建 higher-order 或 init 依赖事实；
    - 已复核 `crates/scoopc/src/llvm/codegen/effect/state_machine_bridge.rs`、`crates/scoopc/src/effect/analysis.rs`、`crates/scoopc/src/effect/state_machine/analysis.rs` 与 `crates/scoopc/src/mir/pass_view.rs`，确认 outward-effect、continuation-escape 与 known-fun suspendability 输入继续来自 shared `collect_known_fun_call_suspendability(...)`、`EffectAnalysisCtx`、`ProgramFacts` 与 pass-view summary / escape facts；LLVM lowering 侧只消费这些 shared facts，不在现场重算 higher-order / init 相关分析；
  - 验证结果：
