...
- 完成说明：
  ...
  - 已新增定向单测：`unified_no_continuation_entrypoint_marks_nosuspend_finally_nested_handle_sample`、`unified_no_continuation_entrypoint_marks_single_nonresuming_sample`；并新增 run-pass fixture：`effect_nosuspend_finally_nested_handle`。
...
### T2003r3b2 [DONE] Effect：统一 emitter 接管 `SingleNonResuming`
...
- 依赖：T2003r3b1
- 完成说明：
  - unified no-continuation 主入口现已扩展到 `UnifiedNoContinuationEntrypoint::SingleNonResuming`，并对 root plan 施加显式 contract 校验：必须恰好一个 `NeverResume` arm，避免 `SingleNonResuming` 静默落回旧 specialized main path。
  - 原先塞在 `codegen_handle_expr` 中的 single non-resuming 主路径已收口为局部 leaf helper `codegen_handle_expr_single_nonresuming_leaf`；`Raise.raise` 与 custom single-payload non-resuming effect 继续复用既有 lowering 细节，但不再承担主选路职责。
  - 已新增定向单测：`unified_no_continuation_entrypoint_marks_single_nonresuming_sample`、`unified_no_continuation_entrypoint_marks_single_nonresuming_finally_nested_handle_sample`；并新增 run-pass fixture：`effect_single_nonresuming_finally_nested_handle`，覆盖 outer single non-resuming + `finally` + nested handle representative sample。
...
### T2003r3b3 [DONE] Effect：统一 emitter 接管 `MultiNonResuming` 并退化旧 specialized entry
...
- 依赖：T2003r3b2
- 完成说明：
  - unified no-continuation 主入口现已扩展到 `UnifiedNoContinuationEntrypoint::MultiNonResuming`，并对 root plan 施加“至少两个 never-resume arms”的显式 contract 校验；pure multi non-resuming handle 不再通过 `codegen_handle_expr_multi_arm(...)` 选主路由。
  - 原先承担 pure multi non-resuming 主路径的 lowering 已收口为局部 leaf helper `codegen_handle_expr_multi_nonresuming_leaf`；它仍可供 mixed/escape 路径的 no-escape fallback 复用，但不再作为 root `MultiNonResuming` 的主入口。
...
### T2003r3d2 [DONE] Effect：先物理删除所有 shape-based resuming legacy 与旧测试，再继续 unified 重写
...
- 依赖：T2003r3d1
- 完成说明：
  - `state_machine_simplify.rs` 与 `nonresuming.rs` 已删除上述 shape-based multi-resuming route 枚举 / match arm / label，只保留统一的 `MultiResuming` 入口占位以及后续 `T2003r3d4` 仍需清空的 `UnsupportedMixedMultiple*`。
  - 仅用于承载旧 multiple-escape resuming 壳层的 `crates/scoopc/src/llvm/codegen/effect/multi_escape.rs` 已物理删除；`mod.rs` / `shared.rs` 中与旧 route 绑定的形状 helper 也已一并删除或收口。
  - `state_machine_plan_tests.rs` 中依赖旧 frame / resolver / route label 的定向单测与 helper 已删除，不再保留 shape-based multi-resuming entrypoint 测试 API。
...
- 验收：
  ...
  - `state_machine_plan.rs` / `shared.rs` 中与 resuming metadata 直接相关的 build-only `todo!` / `unimplemented!` 已清空。
  - 新增或更新 plan 定向单测：覆盖 read tracking、escape capture metadata，以及 plan-driven resolver helper 的 representative direct / indirect / `if` / `while` / nested handle case。
...
- 目标：
  ...
  - `resume(value)` 与 `k.resume(value)` 的 lowering 不再依赖已删除 legacy 文件，而是消费 unified resuming metadata / context。
  - single-resuming representative direct / indirect / `if` / `while` / nested handle 样例由统一 leaf 自然覆盖，不再通过 dedicated shape helper 兜底。
- 验收：
  - `SingleImmediateResume`、`SingleEscapeContinuation` 与 `codegen/mod.rs` 中对应的 build-only `unimplemented!` 已清空。
  - 新增或更新 LLVM 定向单测与 representative run-pass fixture：覆盖 immediate direct、escape direct、escape indirect，以及 `resume(value)` / `k.resume(value)` 的 representative path。
  - 完成后只跑与本任务直接相关的定向验证，例如：
    - `cargo test -p scoopc llvm::codegen::effect::tests:: -- --nocapture`
...
### T2003r3d2c Effect：接回 unified multi-resuming leaf，并禁止任何 shape-based route 名称回流（已拆分，见 `T2003r3d2c1`～`T2003r3d2c3`）
- 拆分说明：
  - 审计 `nonresuming.rs` 当前占位点后确认，原 `T2003r3d2c` 仍同时覆盖三类实现面：
    1. `stack-reentry-only` 的 multi-resuming（多个 immediate-resume arms，可带 sibling non-resuming / `finally`）；
    2. `heap-continuation-only` 的 multi-resuming（多个 escape-continuation arms，可带 sibling non-resuming / `finally`）；
    3. 当前 legal 的 `1 immediate + 1 escape` mixed-resuming 基线。
...
- 目标：
  ...
  - 多个 immediate-resume arms 的 dispatch / replay / `resume(value)` 继续只消费 unified plan resolver / metadata；若仍需 sibling non-resuming dispatch 或 `finally` cleanup，也在同一 leaf 内完成，不回流已删除的 shape-based 命名。
  - 对本子任务尚未覆盖的 `heap-continuation-only` 与 immediate+escape mixed route，给出稳定、显式的非 `unimplemented!` 诊断，并由后续 `T2003r3d2c2` / `T2003r3d2c3` 承接。
- 验收：
  - `MultiResuming` 入口下 `stack-reentry-only` legal route 的 build-only `unimplemented!` 已清空。
  - 新增或更新 plan / LLVM 定向单测与 representative run-pass fixture：覆盖多个 immediate-resume arms，以及至少一例 sibling non-resuming 或 `finally` 组合。
  - 完成后只跑与本任务直接相关的定向验证，例如：
    - `cargo test -p scoopc llvm::codegen::effect::tests:: -- --nocapture`
...
- 目标：
  ...
  - 多个 escape-continuation arms 的 continuation materialization、resume graph、sibling non-resuming dispatch 与 `finally` cleanup 直接消费 unified plan metadata。
  - 代码、tests、fixture、helper 命名不得恢复任何 `T2003r3d2` 已删除的 shape-based legacy。
- 验收：
  - `MultiResuming` 入口下 `heap-continuation-only` legal route 的 build-only 占位已清空。
  - 新增或更新 plan / LLVM 定向单测与 representative run-pass fixture：覆盖多个 escape-continuation arms，以及至少一例 sibling non-resuming 或 `finally` 组合。
  - 完成后只跑与本任务直接相关的定向验证，例如：
    - `cargo test -p scoopc llvm::codegen::effect::tests:: -- --nocapture`
...
- 目标：
  ...
  - immediate dispatch、escape continuation materialization、sibling non-resuming / `finally` 与 resume replay 共用同一套 unified plan contract，不恢复旧 route 名称。
  - `UnsupportedMixedMultiple*` 仍由后续 `T2003r3d3` / `T2003r3d4` 承接；本任务不扩展到更高 arm-count 的 mixed generality。
- 验收：
  - `MultiResuming` 入口下当前 legal `1 immediate + 1 escape` mixed route 的 build-only 占位已清空。
  - 新增或更新 plan / LLVM 定向单测与 representative run-pass fixture：覆盖 current legal `1 immediate + 1 escape` mixed sample，并至少包含 sibling non-resuming 或 `finally` 的 representative 组合。
  - 完成后只跑与本任务直接相关的定向验证，例如：
    - `cargo test -p scoopc llvm::codegen::effect::tests:: -- --nocapture`
...
- 验收：
  - 更新 effect fixtures：至少覆盖 `effect_resume_nested_block_single_perform`、`effect_resume_while_nested_block_perform`，以及届时仍保留的 unified effect nested-block representative samples，统一改为 `do { ... }`，行为保持不变。
