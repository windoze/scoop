...
> 当前状态：P0-P8、`TODO-7-INIT`、P9 全部任务、`P10-T01`、`P10-T01R`、`P10-T02`、`P10-T02R`、`P10-T03-a`、`P10-T03`、`P10-T03R`、`P10-T04-a`、`P10-T04`、`P10-T04-b`、`P10-T05`、`P10-T05R`、`P10-T06` 与 `P10-T06R` 已完成；`P10-T04R` review 发现 cache-hit dependency cone 在下游 stage 不可见的阻塞性缺陷，由 `P10-T04-b`（frontend/effect_facts/mir 中端注入路径，已完成）与 `P10-T04-c`（LLVM codegen 层 dep `LateLoweredProgram` / `.o` artifact handoff，**P10-T06 已部分收口**：dep cone 通过 `scoopc build-single-cone` 子进程产出非空 LIR + `.o` artifact，但 consumer cache-hit 路径仍走 dep-AST-in-active_sources 重排 LLVM stage 而非真正只读 dep `lir_program.bin`）共同覆盖；为支持 per-cone 多进程并发编译，新增 `P10-T05` / `P10-T05R`（CLI 参数与并发抽象）、`P10-T06` / `P10-T06R`（cone DAG 子进程并发 driver），原 P10 全包清场任务顺延为 `P10-T07` / `P10-T07R`；`P10-T05` 已落地 CLI `-j/--jobs` + `SCOOP_BUILD_JOBS` env + `ConcurrencyStrategy` / `SubprocessConeCompiler` trait surface（不引入并发执行行为）；`P10-T05R` review 通过；`P10-T06` 已落地：scoopc 新增 `build-single-cone` 子命令、`scoop` driver 新增 `commands/build/scheduler.rs` 状态机调度器（Pending/InFlight/Done/Failed + cache-hit 短路 + 失败传播 + cone-prefixed diagnostic）、`LocalProcessConeCompiler` 真正 fork+exec scoopc 子进程派发 LocalDependency cone、6 个 driver 单测（包含 `RecordingFakeConeCompiler` 与 `BarrierFakeConeCompiler` 真正验证 trait 解耦 + 并发上限 + 串行回退）、端到端 `cargo run -p scoop -- test` 1507/1507 fixture 全部通过、`source_path_dependency_public_call` 在 `--jobs 1` 与 `--jobs 4` 上 fresh build 与 cache hit 等价产出；`P10-T06R` review 通过：cone DAG 拓扑/失败传播/trait 抽象/jobs=1 与并发覆盖/scoopc 单 cone 边界/cache-hit 与子进程混合下游注入 6 项检查均通过，无阻塞性缺陷；执行顺序：`P10-T05` ✅ → `P10-T05R` ✅ → `P10-T06` ✅ → `P10-T06R` ✅ → `P10-T04-c`（下一步，残余：consumer cache-hit 真正零开销 dep-LIR-bin 注入路径） → `P10-T04R` → `P10-T07` → `P10-T07R`；下一项为 `P10-T04-c`。
