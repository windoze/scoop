...
- 设计原则更新（2026-06-01，见 `PLAN.md` §5「P5 设计原则：effect row 是 dynamic dispatch 的 ABI」）：effect row 推导只用于 static dispatch；dynamic dispatch（itable/vtable）的 target effect ABI 必须已确定。落地方向改为：(a) typecheck 强制 interface（含 default）/`abstract`/`open` 方法的 effectful slot 显式写出 effect row（省略 = `Pure!`），重写/实现 row 必须逐字一致（不许 effect 协变收窄）；(b) dispatch 按 slot（接口实例化后）的固定 row 统一降级，不再 per-impl 解析 effect-varying callable。项目未发布，可直接 breaking change。
- 进展（2026-06-01，工作树未提交，约 15 文件）：已实现 owner-eff 的 typecheck ctor gate / `is_generic` / 实例扫描 / MIR metadata 过滤 / owner-eff 方法 seeding（`owner_eff_param_name`）/ effect-facts owner-eff dispatch substitution（`callable_owner_context` eff_arg 回退）/ boundary operand subtype（`nominal_source_type_compatible` 同 args+eff 走 supertype walk）/ lir-facts `callable_keys_by_instance` 索引。owner-eff 现可 typecheck→monomorph→materialize→dispatch→调用 callback→**effect 正确传播到 handler**。仍卡：dynamic dispatch 的 target InstanceKey eff_args 为空（`{Box.accept,[],[]}`）而 published 实例为 `[Raise<Int>]`，导致 `stable_instance_key` miss、target callable 解析成 `unpublished(Box.accept)`。按新原则，应让 dispatch 直接采用 slot（接口实例化后）的 row 与对应 impl 实例 ABI，而非现在「候选解析失败回落 bare」的路径。详见 `memory/claude_plan.md`。
- 必须实现的内容：
